...
> 状态：Evidence verifier implemented；Places365 ResNet18 **已于 2026-07-24 由项目所有者正式拒绝作生产 standard 包候选**，归档为不可升级的 `research` 证据<br>
> 日期：2026-07-20（初版）；2026-07-24（产品拒绝）<br>
> 适用范围：阶段 5 的生产 standard scene pack 准入；Places365 仅作历史研究档案
...
- **继续把 Places365 ResNet18 当作可关闭的生产候选**（见 §2.4）。

## 2. 候选档案

归档候选固定为 CSAILVision Places365 ResNet18。以下字段与证据仅用于复现既有 `research` 判定，
**不得**再作为生产推进基线；不接受仅凭文件名、默认分支或第三方模型卡“补齐许可”后复活该候选：
...
| provider identity（归档） | `places365-resnet18`；**永久不得**写入可安装 production manifest |
| model identity（归档） | `csailvision/places365-resnet18`；仅出现在 research 报告与证据 |
| preprocessing revision（归档） | `places365-resnet18-center-crop-v1` |
...
- 在权重许可证版本和分发义务被完整记录前，候选不得进入可再分发 App/package。第三方镜像的
  `MIT` 或自称 `CC-BY-4.0` 元数据不能扩大上游权重授权。
...
`.pth.tar`。对已产品拒绝的 Places365 候选，连上述隔离加载也**不再授权**（见 §2.4）。
...
### 2.4 产品处置：正式拒绝作生产包候选（2026-07-24）

项目所有者于 2026-07-24 决定：**Places365 ResNet18 正式拒绝作为 ImageAll 生产 standard 包候选。**

处置边界：

1. **准入机状态**：离线 verifier 对 tracked 报告继续稳定输出 `research`（原因含
   `weights_license_unverified` / `dataset_unverified`）；产品拒绝不等于把报告伪造成
   `rejected` 质量门失败，也不等于补齐许可后可升到 `evaluationReady`；
2. **禁止升级**：不得为该候选补写版本化许可证、下载 Places2/Places365 验证图、加载
   `.pth.tar`、转换 Core ML、生成 standard pack，或替换 `imageall-public-fixture@pack-v1`；
3. **研究输入保留**：`ModelBackend/research/inputs/places365/` 与既有官方字节证据只读保留，
   供审计与复现，永不进入 Xcode resource / App bundle；
4. **公开验证门关闭方式**：不再等待 Places365 专用公开验证集；blocker manifest 改为记录
   “因产品拒绝而永久不适用”，不是“待补齐后即可评测”；
5. **下一生产候选**：必须另开产品决策，选择**许可版本完整、再分发义务可核验、公开评测集许可
   允许本用途**的场景模型；SigLIP2 等超体积 teacher 仍不是首包路径。
...
Places365 ResNet18 的 tracked 报告当前判定为 `research`，并已另加 **产品级永久拒绝**（§2.4）：官方权重
字节 SHA-256 已 verified，但许可版本/再分发义务与公开验证集从未关闭，且项目所有者决定不再尝试关闭。
后续生产标准包必须引入**新的**候选档案，不得复活本归档候选。
...
- 当前 Places365 候选 fixture 虽已复核官方权重 SHA，但因许可证版本/产品分发义务和公开数据 manifest
  未 verified，必须稳定得到 `research`，且不生成 standard pack；2026-07-24 起另记产品永久拒绝，
  禁止任何“补许可后继续评测”的实施切片。
- `ModelBackend/research/manifests/places365-public-validation-20260720.json` 现为产品拒绝后的
  permanent blocker：它固定记录 Places365 公开验证门**永久不适用**，`items` 必须为空。该文件不是
  dataset manifest、评测结果或 `evaluationReady` 证据。
...
后续生产标准包只能针对**新候选**由显式、已审核的许可与公开数据输入推进。Places365 官方许可表述
相互不充分，且已产品拒绝；因此不得加载其 checkpoint、转换或产品接入。新候选若获准进入
`evaluationReady`，runner 也只能在隔离临时目录以 `torch.load(..., weights_only=True)` 使用已批准
只读输入，禁止任意 pickle 回退。
