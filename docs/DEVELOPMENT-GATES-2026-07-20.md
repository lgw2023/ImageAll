...
发布编排，以及生产 GRDB review 事实 adapter、App cache-only adapter、共享激活实例和工具栏原生动作
也已关闭。下一实现门是在用户明确操作当前选中资产时，用既有 App 内 DINO service 生成并写入这一张
资产的版本化 embedding cache；不得扩大为图库扫描或后台预热，不得把 cache miss 隐式变成照片读取，
模型失败仍须与浏览、人工标签和既有 Feature Print 路径隔离。

## 4. 推荐恢复顺序

1. 下一纵切片为当前选中单资产增加显式 App 内 DINO cache 填充动作：只复用既有受控 preview 读取和
   当前完整模型 identity，成功后写一个版本化 embedding；不自动遍历、不批量预热、不入队、不接
   Review Queue 或个人建议展示。自动测试只用程序生成图片。该闭环通过后，才为一个个人标签评估
   `2 + 2` 小批真实标注；任何 HDD2/PhotoKit 读取仍须另列精确只读动作、输出位置和停止位置并取得授权。
   不得恢复 Python/HTTP/helper/XPC 用户依赖；
