...
11. App 内个人线性 head tracer：Swift 从只读 `PersonalModelRebuildSnapshot` 消费内存中的有限合成
    ...
    helper/XPC 和 loopback 运行引用禁入均通过。该 tracer 没有持久化 active artifact，也没有接入
...
cache 及其跨实例原子发布、有界容量/回收、identity 升级清理，以及只消费合成快照的 App 内
Swift/Accelerate 个人线性 head 均已关闭。下一实现门是个人 head 的 App 容器内 managed artifact
生命周期与能力状态；缓存或个人模型故障不得影响浏览、人工标签或既有 Feature Print 路径。

## 4. 推荐恢复顺序

1. 下一纵切片为个人 head 增加 App 容器内 managed artifact 生命周期和独立能力状态：候选 artifact
   必须完整重载与 SHA/identity 校验后才原子切换 active；缺失、损坏、旧 identity 或写入失败保留旧
   active 或安全 unavailable，且只能写 App 自有目录。仍只使用合成快照，不接图库、SQLite、Review
   Queue 或真实照片；
2. 上述生命周期门通过后，再按显式用户动作从当前 App 内 cache/人工事实构造只读训练快照，并独立冻结
   调度、取消和发布契约；此时才需要评估一小批真实人工标注，且任何 HDD2 读取仍须先给出精确只读
   动作和停止位置。不得恢复 Python/HTTP/helper/XPC 用户依赖；
