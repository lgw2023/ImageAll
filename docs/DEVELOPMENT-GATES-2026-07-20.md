...
发布编排也已用合成 adapter 关闭。下一实现门是生产只读 adapter 与显式 Settings/Inspector 动作：只能
查询既有 App embedding cache，不得在 cache miss 时读取图片或运行 Core ML；人工事实只读、发布失败
保留旧 active，且浏览、人工标签和既有 Feature Print 路径不依赖该能力。

## 4. 推荐恢复顺序

1. 下一纵切片为既有 App embedding cache 和人工事实增加生产只读 adapter，并从 Settings/Inspector 的
   显式动作调用已关闭的 rebuild coordinator；cache-only lookup 不接受图片输入、不生成 embedding，
   不新增 migration，不接图库扫描或 Review Queue 展示。该闭环通过后才评估一小批真实人工标注；任何
   HDD2 读取仍须先给出精确 PhotoKit/只读动作和停止位置。不得恢复 Python/HTTP/helper/XPC 用户依赖；
