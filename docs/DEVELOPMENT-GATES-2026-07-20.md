...
状态均已关闭。下一实现门是显式用户动作下，从既有 App embedding cache 与人工事实构造只读训练快照，
并把调度、取消、快照复核和 managed artifact 发布串成最小闭环；缓存或个人模型故障不得影响浏览、
人工标签或既有 Feature Print 路径。

## 4. 推荐恢复顺序

1. 下一纵切片按显式用户动作从当前 App 内 embedding cache/人工事实构造只读训练快照，并冻结单次
   调度、取消、快照二次复核和 managed artifact 发布契约；先继续使用合成 cache/事实，不接图库扫描、
   Review Queue 展示或真实照片。该闭环通过后才评估一小批真实人工标注；任何 HDD2 读取仍须先给出
   精确只读动作和停止位置。不得恢复 Python/HTTP/helper/XPC 用户依赖；
