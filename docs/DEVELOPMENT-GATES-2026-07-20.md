...
1. production standard 准入契约、严格离线 evidence verifier 与候选资源预筛；Places365 曾按证据保持
   `research`，没有下载或伪装成生产批准包；2026-07-24 起已由项目所有者正式拒绝作生产包候选；
...
### A. production standard package：Places365 已产品拒绝（2026-07-24）

官方权重字节 SHA-256 门此前已关闭。2026-07-24 项目所有者决定：**正式拒绝 Places365 ResNet18
作为生产 standard 包候选**。未标版本 `CC BY` 与官方 “academic research / education” 表述不足以证明
产品再分发；Places2/Places365 公开验证集亦不得用于生产准入。该候选归档为不可升级的 `research`
证据，见 [`STANDARD-MODEL-ADMISSION-SPEC.md`](./STANDARD-MODEL-ADMISSION-SPEC.md) §2.4 与
`ModelBackend/research/manifests/`。

因此本门**不再作为“待补齐许可/数据后继续”的开放实施项**。禁止：为 Places365 补许可切片、下载
其验证图、加载 checkpoint、生成 Core ML / standard pack，或把 Places365 artifact 加入 App target。
原始 checkpoint 只读保留在 `ModelBackend/research/inputs/places365/`。

下一生产标准场景包必须另开产品决策，选择许可版本完整、再分发义务可核验、公开评测集允许本用途
的新候选后再走 suggested-only 质量门、Core ML parity/resource/ANE 与 App 内 manifest/身份门。
SigLIP2 等超体积 teacher 仍不是首包路径。
...
2. Places365 生产候选已拒绝。下一标准场景包须先完成**新候选**的产品选型（许可/评测集可核验），再开
   独立准入切片；不得复活 Places365；
