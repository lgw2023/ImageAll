# ImageAll 阶段 1 后端资产闭环架构草案

> 状态：Draft — 已完成后端边界定义，产品界面提案等待项目所有者确认<br>
> 日期：2026-07-15<br>
...
本文只冻结第一个设计面的架构原则和待验收行为。第二个设计面已经触及用户明确保留的前端功能与外观决策；现已形成独立的
[`STAGE-1-PRODUCT-UI-SPEC-DRAFT.md`](./STAGE-1-PRODUCT-UI-SPEC-DRAFT.md)，
但具体提案仍须由项目所有者确认，因此本文在第 14 节建立讨论门并停止：

- 不创建阶段 1 Cursor CLI 实施任务；
- 不修改 Swift、SQL migration、测试、Xcode entitlement 或隐私清单；
- 不选择网格形态、导航结构、标签交互或视觉风格；
- 不访问 `/Volumes/HDD2` 的受保护真实照片。

只有产品界面决策完成、本文转为 Approved，并另行生成自包含实施交接单后，Cursor 才能开始阶段 1。
...
RAW、动态图片、PDF、视频和 Live Photo 当前都不进入首个闭环。JPEG、PNG、HEIC/HEIF、TIFF、WebP、GIF 等格式的精确取舍会影响缩略图行为和用户预期，列入第 14 节产品确认，不在本文静默冻结。
...
缩略图 variant 数量、像素尺寸、裁切方式、色彩背景、动画处理和访问优先级都依赖第 14 节的网格/详情设计。本轮不冻结 cache-entry schema，也不提前创建 v002 thumbnail table。

## 12. migration 与索引方向

`v001_create_catalog_core` 保持完全不变。阶段 1 的实现规格在界面查询与缓存 variant 冻结后，才定义追加 migration。
...
是否新增 thumbnail cache-entry 表和照片网格排序索引，必须等待第 14 节决定。不得因为本文列出“候选”就让 Cursor 修改 schema；正式交接单必须提供精确 DDL、查询证据与反例矩阵。
...
项目所有者提供的前端参考方向已整理为
[`STAGE-1-PRODUCT-UI-SPEC-DRAFT.md`](./STAGE-1-PRODUCT-UI-SPEC-DRAFT.md)。该文档明确区分了阶段 1 的真实闭环与阶段 2–4 的 Review、Compare、Smart Collection、自动化规则和高级调度路线图，并以 `UI-001` 至 `UI-011` 提供可逐项确认的产品提案。

以下问题已经影响模块边界、查询索引、缩略图 variant 和验收标准。它们仍是确认门，不能因为已有提案就由 Codex 或 Cursor 静默视为批准：

### 14.1 信息架构

- 应用打开后首先看到“全部照片”，还是首先看到“来源/文件夹”？
- 是否需要同时存在统一图库与按来源浏览？
- 原始文件夹层级是核心导航，还是只作为筛选条件/元数据？

### 14.2 添加来源与首次使用

- 空目录库的首次使用流程是单页引导、直接系统选取，还是先解释只读与缓存策略？
- “添加来源、停用、重新授权、手动重扫”分别放在哪个功能模块？
- 扫描开始后是否立即进入图库，还是先展示导入/索引进度？

### 14.3 照片浏览与外观

- 网格使用统一方形裁切、完整比例留白，还是可变高度布局？
- 默认缩略图密度、可调范围和详情打开方式是什么？
- 不可用、不可读、格式不支持和缩略图生成中应怎样呈现？
- 日期、文件名、来源、标签和任务状态中，哪些信息常驻显示，哪些按需显示？

### 14.4 选择、标签与筛选

- 单张/多选/范围选择的主要操作路径是什么？
- 标签编辑放在侧栏、检查器、浮层、工具栏还是快捷键流程？
- 多标签筛选默认采用 AND、OR，还是明确提供两种模式？
- 是否需要“无标签、已接受、已拒绝、不可用”等系统筛选入口？

### 14.5 非界面但需同时确认的产品基线

- 首个 MVP 是否正式冻结为 macOS 15+、Apple Silicon only；
- 阶段 1 首轮静态格式允许清单；
- 阶段 1 仍采用本地开发/自用签名，还是现在开始以 Developer ID 或 Mac App Store 为约束。

在以上决策完成前，本草案和产品界面草案均不转 Approved，不创建 Cursor CLI 任务。项目所有者确认后，Codex 再把两份草案收敛为精确的后端实施规格、产品界面实施规格和验收矩阵，并据此拆分全新的 Cursor 会话与切片。
