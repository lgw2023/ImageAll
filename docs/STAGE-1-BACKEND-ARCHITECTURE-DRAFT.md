...
> 状态：Draft — 已完成后端边界定义，等待产品界面共创<br>
> 日期：2026-07-15<br>
...
本文只冻结第一个设计面的架构原则和待验收行为。第二个设计面已经触及用户明确保留的前端功能与外观决策，因此本文在第 14 节建立讨论门并停止：
...
以下问题已经影响模块边界、查询索引、缩略图 variant 和验收标准，必须与项目所有者共同决定，不能由 Codex 或 Cursor 静默选择：
...
在以上决策完成前，本草案不转 Approved，不创建 Cursor CLI 任务。讨论完成后的下一份文档应是“阶段 1 产品界面规格 + 后端实施规格”，并据此拆分全新的 Cursor 会话与切片。
