...
> 日期：2026-07-22；界面收敛修订：2026-08-04
...
| TW-P8 | 对齐图库瘦身的高密度工作台：媒体、主操作、模型状态、记录开关、刷新与运行反馈合并为单层命令栏；Run 导航可收起；技术 JSON 默认折叠 | **已批准**（所有者要求，2026-08-04） |
...
```text
┌──────────────────────────────────────────────────────────────────────────┐
│ 照片/视频 | 新建任务 | 模型 x/3 | 显隐记录 | 刷新 | 运行反馈 | 返回图库       │
├──────────────────┬───────────────────────────────────────────────────────┤
│ Run 导航（可收起）│ Run 详情（最大内容宽度 1000，超宽窗口不拉伸字段）         │
│ · 方法 / 状态     │ 标题 + 状态 | 时间/媒体账本 | 过程 | 产物                │
│ · 时间 / 方法筛选 │ 数据、配置、结果与完整标识统一收进默认折叠的技术详情       │
└──────────────────┴───────────────────────────────────────────────────────┘
```

命令栏采用与图库瘦身一致的控制优先级：唯一高强调动作是「新建训练任务」；模型槽
以 `模型 x/3` 保持总体状态常驻，展开后逐项显示三轨状态；训练活动在右侧行内显示，
不再用整行 Banner 推动工作区内容。Run 列表只显示方法、状态、技术名与时间，完整 UUID
保留在悬停帮助和技术详情中。

### 4.3 三槽位状态条（全并行的用户可见锚点）

命令栏始终显示三轨中已有多少 **published** 产物，并可展开查看每一轨的精确状态：
