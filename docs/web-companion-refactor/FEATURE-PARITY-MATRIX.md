...
| 审查概览 | Mac 有来源范围、每标签上限、本地模型状态/任务和分组折叠 | 标签卡、计数、可定址来源范围、共享每标签上限、本地模型双轨控制和进入队列 | 复用 Host 标签树，保留本地折叠偏好和稳定键盘导航 | 概览、`tags`、`tag-groups`、精确来源、通用设置和 `librarySuggestions` 双轨任务投影已有 | 无 | parity-proven | `curation.spec.ts`：Host `groupID`/`sortOrder` 分组、未知标签回退、折叠重载、方向键与 Home/End；全部/部分/零来源→队列→返回；上限 200→503→250；标准/个人任务控制和失败重试，桌面+移动、axe | `evidence/curation/imageall-react-review-{groups,source-scope,limit,models}-chromium-{desktop,mobile}.png` | 合成标签树、服务和任务证明界面投影与交互；不证明真实模型推理、吞吐、建议质量或人工 VoiceOver 听读 |
