...
| 审查概览 | Mac 有来源范围、每标签上限、本地模型状态/任务和分组折叠 | 标签卡、计数、可定址来源范围、共享每标签上限、本地模型双轨控制和进入队列 | 补齐按标签组折叠与导航语义 | 概览、精确来源、通用设置和 `librarySuggestions` 双轨任务投影已有 | 尚缺 overview 标签组映射 | in-progress | `curation.spec.ts`：全部/部分/零来源→概览计数→队列→返回；上限 200→503→250；服务刷新→标准启动→暂停/恢复/取消→个人启动，503 可重试且保留来源；概览 503→重试，桌面+移动、axe | `evidence/curation/imageall-react-review-{source-scope,limit,models}-chromium-{desktop,mobile}.png` | 来源范围、共享上限和模型任务主路径已证明；合成服务/任务不证明真实模型推理、吞吐或建议质量，未补齐分组前不得外推为 Mac 审查概览全功能对齐 |
