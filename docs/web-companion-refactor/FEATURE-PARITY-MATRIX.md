...
| 审查队列/连续单图/决策/undo | Mac 网格选择、Space 单图、P/X/U 连续判断、方向键、来源/密度/宽高比 | 单项/批量、连续单图、undo、与概览共享来源范围 | 保留批量与范围上下文，继续补齐密度/宽高比和云端预览 | 已有 | 无来源筛选缺口；其余无 Host 缺口 | in-progress | `curation.spec.ts`：来源范围跨路由保持；单项/批量/undo；Space→U→P→自动下一张→Esc，桌面+移动、axe | `evidence/curation/imageall-react-review-{source-scope,single-photo}-chromium-{desktop,mobile}.png` | 合成 8 项已证明来源与连续单图主路径；网格密度/宽高比和 iCloud-only 审核预览仍未对齐 |
