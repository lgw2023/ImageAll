...
| 审查概览 | Mac 有来源范围、每标签上限、本地模型状态/任务和分组折叠 | 标签卡、计数、可定址来源范围、共享每标签上限和进入队列 | 补齐生成/暂停/恢复/取消、本地模型状态和分组语义 | 概览与精确来源参数、通用设置读写已有；任务能力分散在 training API | 尚缺面向 Review 的统一模型能力投影与标签组映射 | in-progress | `curation.spec.ts`：全部/部分/零来源→概览计数→队列→返回；上限 200→503 不假更新→250→队列往返保持；概览 503→重试，桌面+移动、axe | `evidence/curation/imageall-react-review-{source-scope,limit}-chromium-{desktop,mobile}.png` | 来源范围与共享上限主路径已证明；合成设置响应不证明真实四轨任务实际采用该上限，不得外推为 Mac 审查概览全功能对齐 |
