...
| 审查队列/连续单图/决策/undo | Mac 网格选择、Space 单图、P/X/U 连续判断、方向键、来源/密度/宽高比 | 单项/批量、连续单图、undo、与概览共享来源范围、9 档密度和缓存优先原比例 | 保留批量、范围和呈现上下文，继续补齐云端预览 | 已有 | 无来源、密度或宽高比 Host 缺口 | in-progress | `curation.spec.ts`：来源范围跨路由；密度/比例 URL 恢复、原比例缩略图请求、不重拉队列/不丢当前项；单项/批量/undo；Space→U→P→自动下一张→Esc，桌面+移动、axe；Swift 缓存命中/正方形回退 | `evidence/curation/imageall-react-review-{source-scope,single-photo,view-controls}-chromium-{desktop,mobile}.png` | 合成 8 项已证明来源、连续单图和呈现上下文；原比例只证明缓存请求/回退契约，不证明真实照片缓存命中；iCloud-only 审核预览仍未对齐 |
