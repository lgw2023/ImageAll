...
| 筛选 | Mac 支持标签包含/排除、ALL/ANY、已标记状态、可用性和格式 | React 回归后仅剩单个“已接受标签”；旧 Web 曾有完整条件模型 | 独立 Web 视觉的响应式条件构建器；完整状态可定址并精确交给 Host/当前筛选分析 | 已有，无协议扩展 | 无 | parity-proven | `gallery.spec.ts`：混合包含/排除、ALL/ANY、已标记/未标记、可用性、格式、URL 重载、一键清除、桌面/移动无重叠和 axe；`slimming.spec.ts`：完整筛选精确移交分析 | `evidence/gallery/imageall-react-advanced-filters-chromium-{desktop,mobile}.png` | 合成 Host/资产；证明交互、查询和响应式布局，不证明真实目录格式统计或人工视觉偏好 |
...
| 普通图库删除 | Mac 检查器与批量操作使用破坏性确认；文件原片可永久删除，Photos 资产进入“最近删除”，收藏受保护 | 批量入口误标为“移至回收区”并提交真实 Host 不接受的 `recoverableRecycle`；单图查看无入口 | 单图与冻结多选共用应用内破坏性确认；提交精确 `gallerySelection + releaseSourceSpace`，保留 Mac 二次确认、焦点返回、键盘与失败重试 | `/v1/library-slimming/removals` | 无 | parity-proven | `gallery.spec.ts`：单图精确 payload、Delete/Esc、取消零写入、焦点恢复、409 原位重试、axe；`slimming.spec.ts`：冻结多选、应用内确认与精确 payload，桌面+移动 | `evidence/gallery/imageall-react-deletion-confirmation-chromium-{desktop,mobile}.png` | 合成 Host；没有删除真实 Photos/文件，最终破坏性执行仍须 Mac 确认；聚焦 Swift runner 本轮未拉起，不能据此声称 Swift 用例通过 |
...
| 图库上下文检视器 | Mac 右侧检视器显示所选照片元数据、标签和上下文动作 | 工作区外壳仅显示会话信息，并以占位文案承诺未来显示所选照片属性 | 单图显示 Host 权威完整元数据；冻结多选显示来源范围、标签状态以及查看/收藏/删除/清空动作；桌面浮层与移动抽屉保持选择和焦点连续 | `assets/:id`、`tags`、`tag-groups`、收藏、删除与批量标签决定已有 | 无 | parity-proven | `gallery.spec.ts`：单图元数据、冻结多选、确认/拒绝/清除、同 operation ID 失败重试、收藏/删除/清空、关闭焦点返回、44 px 触控、无溢出和 axe，桌面+移动 | `evidence/gallery/imageall-react-context-inspector-chromium-{desktop,mobile}.png` | 合成 Host/资产/标签；证明投影、请求与交互，不证明真实照片元数据准确性、大标签库性能或人工 VoiceOver 听读；自动测试未读取 HDD2 |
| 单图标签检查器 | Mac 按 Host 标签树分组，支持折叠与快速决定 | 平铺标签与三个小按钮 | Host 权威分组、共享折叠偏好、标签主体快速操作与稳定焦点 | `assets/:id`、`tags`、`tag-groups`、批量标签决定已有 | 无 | parity-proven | `gallery.spec.ts`：Host `groupID`/`sortOrder`、组折叠重载、方向键/Home/End、左键确认、右键清除、X 拒绝、Backspace 清除、写入后焦点恢复，桌面+移动、axe | `evidence/gallery/imageall-react-inspector-tags-chromium-{desktop,mobile}.png` | 合成 Host 和标签树；证明交互与投影，不证明真实大标签库性能或人工 VoiceOver 听读 |
...
| 审查概览 | Mac 有来源范围、门槛以上全部保留、本地模型状态/任务和分组折叠 | 标签卡、计数、可定址来源范围、无数量上限策略、本地模型双轨控制和进入队列 | 复用 Host 标签树，保留本地折叠偏好和稳定键盘导航 | 概览、`tags`、`tag-groups`、精确来源和 `librarySuggestions` 双轨任务投影已有 | 无 | parity-proven | `curation.spec.ts`：Host `groupID`/`sortOrder` 分组、未知标签回退、折叠重载、方向键与 Home/End；全部/部分/零来源→队列→返回；明确显示门槛以上全部保留且无上限控件；标准/个人任务控制和失败重试，桌面+移动、axe | `evidence/curation/imageall-react-review-{groups,source-scope,retention,models}-chromium-{desktop,mobile}.png` | 合成标签树、服务和任务证明界面投影与交互；不证明真实模型推理、吞吐、建议质量或人工 VoiceOver 听读 |
...
| 存储与维护 | Mac 设置 | 清理、导出、迁移、重启均可达 | 容量、健康、操作请求、冲突锁、待处理状态和完整结果投影；应用内确认与稳定操作 ID 重试 | 已有 | 无 | parity-proven | `management.spec.ts`：四类动作、清理确认、不可用原因、活动请求锁、外部迁移、失败原位重试和 Host 全结果；桌面+移动无碰撞 | `evidence/management/imageall-react-storage-vault-chromium-{desktop,mobile}.png` | 只验证合成 Host 请求与投影；不证明真实磁盘回收、真实导出、迁移或 App 重启，自动测试未读取 HDD2 |
...
| 通用设置 | Mac Settings 支持模型、预热、工具栏、待审上限、三轨默认阈值、按标签覆盖/参考值/继承恢复与低分待审清理 | Web 已补齐同一组 Host 权威设置动作 | 类型表单、单 mutation 部分更新、Host 回读、可搜索按标签工作台、清理确认和稳定 operation ID 重试 | 已有；`settings/general` 已含四类阈值 mutation | 无 | parity-proven | `management.spec.ts`：普通部分更新、三轨 `setDefault`、`setOverride`、参考值、`clearOverride`、`prune`、503 同 ID 重试、409 冲突；桌面+移动、axe、无溢出、44 px 触控 | `evidence/management/imageall-react-suggestion-thresholds-chromium-{desktop,mobile}.png` | 合成设置/标签/参考值；不证明真实 Mac 持久化、真实分数质量或清理后的模型效果，自动测试未读取 HDD2 |
