...
20. Web Companion 的位置更新来源卡按稳定 source ID 协调；同一来源从开始、提交到可取消阶段必须复用
    同一动作按钮，并在动作终结时把焦点移交给最近仍可操作的来源。长来源列表只保留一个 roving Tab
    停靠点，方向键与 Page Up/Down 只改变本地焦点，不读取位置目录或提交扫描命令。
