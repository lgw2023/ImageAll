...
三个切片通过后，阶段 4 仍未宣称完整完成。10 万合成元数据基准已经按第 6 节关闭；100 万合成元数据
基准、1 万独立基线、缓存压力测试、发布前隐私/故障回归、三个真实标签校准以及真实 macOS 权限
回归继续保持独立验收门；其中任何真实图库或真实照片验证都需要项目所有者针对具体来源重新授权。
...
- 仓库与临时 fixture 位于外置 USB 固态盘 `SSD1`，APFS；
...
或真实目录 I/O，也不据此调整生产 SQLite 参数。下一独立规模门先用同一确定性 fixture 补齐 1 万独立
基线并扩展至 100 万条，验证查询正确性和响应上限，再决定是否把约 10 倍体积的可移植导出纳入常规
完整测试。
