...
> 状态：Core implementation complete；Slice X 百万资产迁移容量校准已关闭
>
> 决策日期：2026-07-17
>
...
已经按第 16 节关闭，百万级 v005→v006 迁移耗时、峰值工作副本空间与容量预检已经按第 17 节关闭；
Apple Photos 权限回归与三个真实标签运行校准随后已在项目所有者明确授权的签名 App 会话中关闭，证据
分别见阶段 2 PhotoKit 规格第 8.1 节和阶段 3 Review Queue 规格第 8.1 节。真实摄影格式/内容分布及
端到端大容量图片 I/O 继续保持独立验收门；其中任何新的真实图库或真实照片验证都需要项目所有者针对
具体来源重新授权。

“三标签运行校准已关闭”只表示既定只读测试和能力记录完成，不代表当前轻量基线必须达到统一准确率、
样本完整性或标签覆盖率。它们不再是阶段 4 或 MVP 发布门；后续 Hugging Face 通用语义模型评估是独立
阶段，不能反向弱化本阶段的可恢复性、只读保护和人工事实优先契约。
...
源代码静态检查冒充运行时权限证明。在本 Slice 验收时，真实 macOS 用户会话权限回归、真实 UI/图片 I/O
与端到端容量门、三个真实标签校准仍然开放；其中权限回归与三标签运行校准随后已按独立授权关闭，
真实摄影内容与大容量图片 I/O 仍需单独授权。
...
100 个合成有效 JPEG 的生产后端管线基线，不能升级为真实 UI 或端到端大容量图片 I/O 已验收。在本 Slice
验收时真实 macOS 权限回归和三个真实标签校准仍然开放；两者后来已按独立授权关闭。
...
## 17. Slice X：百万资产 v005→v006 迁移容量校准

状态：Completed，实施 commit `2fa3702662b8cfcc79cf9795e58793655fd4a968`。

本 Slice 使用支持 1 万日常烟测和 100 万显式档位的同一合成目录库生成器，先建立完整 v005 数据库，
再通过生产 `CatalogBootstrapCoordinator` 执行 checkpoint、同卷工作副本迁移、原子替换、替换后校验和
旧库保留。100 万档只通过编译条件 `IMAGEALL_MIGRATION_MILLION` 显式启用，默认全量回归仍运行 1 万档，
避免日常测试意外承担约 19 分钟的规模门。

校准测试同时验证：

- live 数据库包含全部已知 migration，`PRAGMA quick_check` 通过，Asset、Source、人工决定和 FTS 行数
  与生成器一致，代表性 FTS 查询返回原 Asset；
- `pre-restore` 旧库仍保持 v005 migration 历史和 100 万 Asset，证明迁移只发生在工作副本；
- 每 10 毫秒采样测试拥有的临时根目录，记录逻辑字节、allocated bytes、文件系统空闲空间和
  `volumeAvailableCapacityForImportantUsage`；测试根若解析到 `/Volumes/HDD2` 或仓库 `user/` 会在创建
  fixture 前拒绝；
- 容量要求由 `2 × max(source footprint, 1 MiB) + 64 MiB` 调整为
  `3 × max(source footprint, 1 MiB) + 64 MiB`，乘法与加法继续溢出失败关闭；空间不足、工作副本迁移
  失败和替换后校验失败的既有原子性回归保持通过。

2026-07-17 的 100 万档在 Mac mini、arm64、macOS 26.5.1 上通过，原始结果位于
`/Volumes/SSD1/ImageAll-Stage4X-1M-20260717-1716.xcresult`，保留附件位于
`/Volumes/SSD1/ImageAll-Stage4X-1M-Attachments-20260717-1736`：

- XCTest 单项 1/1 通过，用时 1136.336 秒；其中生产启动迁移用时 850.884 秒；
- v005 source footprint 为 1,079,873,536 字节，v006 live footprint 为 1,079,922,816 字节；
- 基线逻辑占用 1,079,873,536 字节，峰值逻辑占用 3,339,260,495 字节，峰值新增
  2,259,386,959 字节；峰值 allocated bytes 为 3,336,384,512；文件系统空闲空间最大下降
  2,367,328,256 字节，共采样 27,451 次；
- 旧公式要求 2,226,855,936 字节，比逻辑峰值少 32,531,023 字节，且比文件系统空闲空间下降少
  140,472,320 字节；新公式要求 3,306,729,472 字节，分别保留 1,047,342,513 字节与
  939,401,216 字节余量。`volumeAvailableCapacityForImportantUsage` 在本次观测中未下降，因此不单独用它
  替代目录逻辑峰值与文件系统空闲空间证据。

提交 `2fa3702` 的干净归档、不含 `user/` 或未提交 App 图标资源的最终门为：

- unsigned arm64 Debug 串行 tests 813/813：
  `/Volumes/SSD1/ImageAll-Stage4X-CleanFullUnsigned-20260717-1748.xcresult`；
- Apple Development entitlement 1/1：
  `/Volumes/SSD1/ImageAll-Stage4X-CleanSignedEntitlement-20260717-1749.xcresult`；
- signed arm64 Debug build：
  `/Volumes/SSD1/ImageAll-Stage4X-CleanSignedBuild-20260717-1750.xcresult`，产物由 Team
  `962554J6D3` 的 Apple Development 身份签名并通过 `codesign --verify --deep --strict`。

自动化只创建和删除测试自有的合成临时数据库，不读取图片字节、不调用 PhotoKit、不读取 `user/`，
不访问或遍历 `/Volumes/HDD2`。本 Slice 关闭当前 v005→v006、100 万合成事实在此设备上的容量校准门；
850.884 秒不是产品 SLA，新公式也不外推为未来 schema、不同事实分布、不同文件系统或真实照片 I/O 的
通用空间保证。三个真实标签运行校准与 Apple Photos 权限回归后来已按独立授权关闭；真实摄影内容分布
与端到端大容量图片 I/O 仍需项目所有者针对具体数据源授权后独立验收。
