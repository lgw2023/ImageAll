...
> 状态：标准场景 fixture tracer、standard HTTP 与个人训练 CLI 已实现；personal HTTP 和生产公共模型仍待后续门
> 日期：2026-07-18
...
带 catalog、bundle、encoder、标签词表和权重 hash 身份的只读 bundle。该切片不包含 App 侧快照导出、
personal `/v1/suggestions` 或生产照片训练。
...
截至 `abc5ef0`，门 1、2、3、5、6、7 和门 4 的 standard 半边已在公开合成 fixture 或合成 embedding
层关闭；个人 bundle 加载同时校验 catalog、bundle revision、DINO encoder 和标签词表。personal
