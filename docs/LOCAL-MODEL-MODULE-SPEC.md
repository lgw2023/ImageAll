...
> 状态：Python/HTTP 开发验证链、DINOv2 Core ML 转换/资源证据及既有双轨实验契约已实现；正式部署方向已切换为 App 容器内 Swift/Core ML，固定 artifact tracer、App 内启用选择、版本化 embedding cache、资源生命周期、个人线性 head、managed artifact、生产只读 adapter 与显式原生重建均已实现；Places365 官方权重字节已复核，许可、公开数据和生产准入仍待后续门
> 日期：2026-07-20
...
### 2026-07-20 部署决策（覆盖本文件中的旧 App loopback 规划）

正式 App 统一采用本地 Core ML：用户在 App 内选择启用模型，App 在自身容器中完成许可证、版本、
SHA-256 与 manifest 校验、初始化和缓存，Swift 直接加载 Core ML artifact，由 Core ML 使用本机
CPU/GPU/Neural Engine。App 全程不启动 Python、loopback HTTP、helper/XPC，也不依赖 `uv` 或工作区
`.venv`。`ModelBackend` 和下文 HTTP 契约仅保留为转换、离线评测及开发验证工具；既有提交与证据是
历史可复现记录，不是正式分发架构。Developer ID 与 Mac App Store 的选择不阻塞 App 内模型接线。

## 1. 目标与停止位置

本模块为 ImageAll 提供可替换的 App 内本地视觉模型能力，但不是 App 的启动依赖。能力分为零用户样本即可
工作的标准标签公共模型，以及只从用户人工正负决定学习的个人标签模型。模型未启用、artifact 未安装、
权重缺失或单次推理失败时，App 仍须保留浏览、人工标签、历史标签预设和现有 Vision Feature Print
个性化建议闭环。

第一条 App 内 tracer slice 已完成：

```text
程序生成图片
→ App 容器内 manifest / SHA-256 / identity 校验
→ Swift 直接加载固定 DINOv2-small Core ML artifact
→ 384 维有限且身份匹配的 embedding
→ 未启用、缺失或损坏时安全降级
```

第一切片停止于 Swift 公开接口测试、程序生成图片真实 embedding smoke、实际 App bundle 加载和 App
build；生产 target 不再编译 loopback client，旧 client 仅保留在测试/开发验证 target。不修改 SQLite
schema 或人工标签事实，不下载 SigLIP2、DINOv3、FastViT 或 Ollama VLM 权重。个人模型后续优先在
App 内复用同一 DINO encoder，并训练/推理轻量线性 head。

第二条 App 内能力管理切片也已完成：

```text
全局启用意图（默认关闭）
→ Settings 立即显示 disabled / validating
→ actor 在非主线程串行校验并加载固定 bundle artifact
→ ready 显示完整 model identity；失败显示去敏 unavailable
→ 关闭释放服务；重启或 artifact 更新重新校验
```

该切片没有把模型接入 LibraryWorkspace、图库扫描、SQLite、Review Queue 或个人模型；任何失败继续与
浏览、人工标签和既有 Feature Print 闭环隔离。

第三条 App 内版本化缓存切片已完成：

```text
显式程序生成图片 + catalog/asset/content revision
→ 固定 Core ML identity 与输出语义校验
→ App Caches 内有限 little-endian Float32 + SHA-256 原子缓存
→ exact hit 复用；revision/身份变化 miss；损坏或 NaN 重建
→ 模型关闭不写缓存；缓存故障退化为实时 embedding
```

App manifest revision 2 固定 `dinov2-cls-token`、`raw-float32-v1` 和 `float32`；cache address 还包含
provider/model revision、preprocessing、完整 artifact/manifest/license provenance。该切片停止于程序
生成单图、同实例并发收敛、损坏恢复和生产 Composition Root；没有接入 LibraryWorkspace、图库扫描、
SQLite、Review Queue 或个人模型。

第四条 App 内缓存资源与生命周期切片也已完成：

```text
不同 cache 实例/进程请求
→ App 自有 lifecycle 文件锁覆盖 lookup、推理、原子发布与回收
→ record schema 2 / 完整模型 identity 首次清理
→ 默认 256 MiB 对象预算，只淘汰 ModelEmbeddings 自有常规对象
→ 任一缓存基础设施失败继续返回实时有限 embedding
```
...
```text
工具栏显式重建
→ review 只读人工事实 + App cache-only 精确身份查询
→ 复用 Settings 已激活的同一 AppCoreMLEmbeddingService
→ Swift/Accelerate 训练并原子发布 managed head
→ 模型未启用或 cache miss 时独立提示，浏览/人工标签不受影响
```

cache-only adapter 不接受图片，不生成 embedding，不在 miss 时创建目录；组合根不创建第二份模型，正式
路径也不恢复 Python/HTTP/helper/XPC。该切片没有接图库扫描、Review Queue、个人建议展示或真实照片。
下一条推荐门是只对用户当前选中的单个资产增加显式 App 内 cache 填充动作，自动测试继续只用程序生成
图片；在该门完成前不需要真实标注。
...
每个合格标签最近的人工接受/拒绝决定构造 embedding 快照，由用户显式触发同步重建；成功缓存的
embedding 还可在后续人工反馈后驱动 30 秒防抖的持久 cache-only 自动重训。后台 job 不含图像、路径、
bookmark 或 bytes，快照过期即跳过，缓存 miss 保留显式同步重建入口，且不会启动离线服务。该闭环不
包含生产照片准确率结论。用户也可从 Review Overview 显式触发 personal 全库扫描；App
...
`/v1/suggestions`。同一固定 artifact 已作为首发小模型接入 App target；App 只在用户启用后通过 Swift
工厂校验并加载，模块缺失或损坏时的原有降级边界不变。HTTP 路径仍只属于开发验证工具。
...
| DINOv2-small | 个人标签冻结 embedding | 固定 revision 的 PyTorch/Core ML 开发验证、FP16 artifact、App 内 Swift 加载/启用选择/版本化 embedding cache、跨实例发布、256 MiB 预算和旧 identity 清理均已实现；Swift/Accelerate 个人线性 head、App Application Support managed artifact、生产只读 adapter 与显式原生重建均已实现；当前选中单资产的显式 cache 填充待下一切片 |
...
- 正式 App 只从自身 bundle 或 App 容器读取已批准 artifact；首发小模型可以随 App 分发，较大可选模型
  的受校验按需下载机制另行设计；
- 启用前必须核对许可证登记、manifest schema、provider/model/preprocessing identity、版本、文件清单
  与 SHA-256；任何缺失、损坏或不匹配都返回稳定 unavailable，不启动外部进程或跨 provider 回退；
- Swift 使用 Core ML 公开 API 直接初始化和推理，不经过 HTTP；模型结果只能进入可重建机器结果，
  浏览、人工标签和人工事实事务不依赖模型初始化；
- Python/uv loopback 只属于开发工具，仍固定 `127.0.0.1`，不得进入 App bundle、启动链或普通用户文档；
...
- App 内 Core ML loader 还必须核对固定 `1×3×224×224` 输入与 `1×384` 输出；artifact 缺失、身份/
  校验和/输入输出不匹配或 runtime 不可用时 fail closed，禁止自动回退 PyTorch、HTTP 或其他轨道；
...
实验接入；本文件定义的版本、身份和失效契约不变，HTTP 只保留为开发验证契约。
...
## 3. 开发与离线验证 HTTP 契约（非 App 运行契约）

本节记录既有 `ModelBackend` 测试工具的可复现接口。正式 App 不调用这些 endpoint，也不要求用户安装
或启动该服务。身份字段、失败语义和 fixture 可继续用于转换/parity/离线评测，但新的 Swift 接线必须
通过 App 内公开接口独立验证。
...
这是 `f8ea5e7` 当时的历史状态；后续 `e728bcd` 已把 personal 全库扫描迁入统一持久 job/checkpoint，
不再是当前缺口。
...
2. 个人训练：独立 CLI 与 App 用户触发的版本化 embedding/decision 快照、同步重建、原子发布和热重载
   ...
   Queue；人工反馈后的 30 秒防抖、持久、cache-only 自动重训已由 `42b6423` 完成，冻结 payload 不含
   私有图片字段，快照过期跳过，缓存 miss 保留手动回退，服务和 CAS 故障可重试；App 内 Swift 路径的
   生产事实/cache-only adapter、共享模型激活实例和显式重建动作已由 `173a117` 关闭，下一门只处理当前
   选中单资产的显式 cache 填充，不恢复后台自动重训或 loopback 用户依赖；
...
4. Core ML FP16：Python 转换、artifact 校验、数值一致性、CPU_ONLY/ALL 请求基准与只读 compute
   plan/resource 报告已完成；固定 DINOv2 artifact 的实际 Neural Engine 分配、预编译安装、峰值 RSS、
   1,000 次稳定性、热状态及独立 uvicorn 的 ready/合成 embedding/关闭生命周期已通过去标识 evidence
   验收；该结论不能外推到未来 standard 模型；
...
6. 旧 Swift loopback client：其严格解码和身份门仅在测试/开发验证 target 保留，不进入正式 App；App
   内固定 Core ML artifact 的 manifest/SHA/identity 校验、有限 embedding 和无模型降级门已关闭；
7. 再独立评估 SigLIP2、区域证据、FastViT student 和可选 Ollama VLM adapter。

当前选中单资产 cache 填充、SigLIP2 下载、Ollama 模型下载、生产 standard 模型和真实照片人工验证都
必须按独立切片重新冻结范围和验收门。
...
- 不读取或写入受保护真实照片路径；
- 不遍历 `.photoslibrary`；
- 不让 Python、uv、loopback HTTP、helper/XPC 或 Ollama 成为 App 的链接、启动、推理或数据库依赖；
...
ready 后开始，端到端“进程启动 → 服务 ready”当时保留为独立开放门；该门已由下文 2026-07-20
启动探针关闭。`4d570c2` 增加原子
...
2026-07-20 独立服务进程端到端验收：`7e0874b` 新增
`imageall-verify-service-startup`，只允许显式 `127.0.0.1` endpoint、以 argv 启动目标而不经过 shell，
轮询严格 health/capability，并按 `control`、`embedding` 或 `standard` 发送程序生成 PNG；证据不记录
完整命令或 artifact/cache 路径，所有退出路径都确认进程已停止。Backend 全量为 `182 passed, 2
skipped`，compileall、sdist/wheel 和 console entrypoint 通过。随后从既有 `/tmp` 离线模型缓存重新转换并
预编译固定 DINOv2 revision，实际 uvicorn 在约 3.087 秒公布 ready，合成 PNG 返回 384 维有限
float32 embedding，SIGTERM 后退出且端口无监听残留。去标识结果见
...
这关闭独立服务的“进程启动 → ready → HTTP 推理 → 关闭”门，不代表 App 已内嵌、安装或自动托管服务。

同日 App 内模型能力管理验收：`573add4` 以 TDD 交付全局默认关闭的 UserDefaults 启用意图、串行
`AppModelActivationCoordinator` actor、原生 Settings 状态和生产 bundle 组合根。能力管理定向
`11/11`、既有 App Core ML artifact `6/6`、组合根 `5/5` 通过；固定 artifact 的初始化未在主线程执行，
重复/并发启用只创建一个服务，关闭释放服务，显式 off-on 才重试，新生命周期会按持久意图重新校验。
完整直接 xctest 执行 958 项，只有 3 个依赖 App 宿主身份的测试产生 6 个环境断言失败；受保护图库挂载
期间没有启动 App 宿主。Release App build 成功，包内 manifest/license 与源 SHA 一致，Python、原始
checkpoint、helper/XPC 和 loopback endpoint 禁入计数均为 0；临时包使用项目 entitlements ad-hoc
签名后通过 strict codesign。测试与构建未读取 `/Volumes/HDD2` 或任何 `.photoslibrary` 内容。

同日 App 内版本化 embedding cache 验收：`393e0dc` 将 App manifest 升为 revision 2 并固定
`dinov2-cls-token`、`raw-float32-v1`、`float32`，新增只写 App Caches 的完整身份缓存。TDD 实际看到
缺身份字段、缺缓存公开接口、缺生产工厂和 8 次并发全部 generation 的 RED，随后分别 GREEN；cache
`7/7`、Core ML `6/6`、能力管理 `11/11`、Composition Root `6/6` 通过。完整非宿主 xctest 执行 967 项，
仍只有 3 个 App 宿主身份测试的 6 个环境断言失败。Release build、strict codesign、manifest/license
源包 SHA 一致和 Python/原始权重/helper/XPC/loopback 禁入计数均通过。所有图片均由程序生成，未启动
App 宿主，未读取 `/Volumes/HDD2`、`.photoslibrary` 或真实照片。

同日 App 内 embedding cache 资源与生命周期验收：`f2cac33` 将 cache record schema 升为 2，以进程锁
和 App 自有 POSIX lifecycle 文件锁串行化不同实例/进程的 lookup、Core ML inference、原子发布与回收；
默认对象预算为 256 MiB，只统计和删除 `ModelEmbeddings/v1/objects` 下的常规 `.embedding` 文件。TDD
逐项观察到跨实例 `8 generated / 0 hit`、缺预算接口、旧 schema 孤儿残留和旧 identity 孤儿残留四个
RED，随后 GREEN；cache `11/11`，连同 Core ML service、能力管理和 Composition Root 为 `34/34`。
...
同日 App 内只读重建编排与生产接线验收：`6f8e819` 先以合成 adapter 关闭单运行、取消、发布前快照复核
和 cache miss/error fail-closed；`173a117` 再把 review 人工事实与完整身份 App cache-only lookup 接入
生产组合根，并让 Settings 与 workspace 共享同一已激活 Core ML service。工具栏显式动作只运行 App 内
Swift/Accelerate trainer 和 managed store；未启用不创建 cache/head，cache miss 不读取图片且显示独立
提示。定向 cache `13/13`、coordinator `9/9`、activation `12/12`、Composition Root `7/7`、store
`6/6`、head `5/5`、workspace `106/106`。完整非宿主 xctest 为 997 项，仍只有既有 3 个宿主身份测试
产生 6 个环境断言失败；Universal Release build、strict codesign、资源一致性和生产包禁入门均通过。
未启动 App 宿主，未读取 `/Volumes/HDD2`、`.photoslibrary` 或真实照片，也未接图库扫描、Review Queue
或个人建议展示。
