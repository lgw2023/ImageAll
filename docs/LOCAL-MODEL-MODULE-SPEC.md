...
> 状态：First tracer slice implemented；双轨后续切片契约已冻结
> 日期：2026-07-18
...
1. 标准场景 tracer：公开 fixture → 固定公共模型 → mapping → ontology DAG → suggestion policy；
2. 个人训练 CLI：从 ImageAll 显式导出的版本化 embedding/decision manifest 训练并重载 bundle；
3. `/v1/suggestions`：双轨响应、稳定错误和无 final-label 语义；
