...
> 状态：标准/个人 suggestion HTTP、双轨服务启动、个人训练 CLI、DINOv2 Core ML FP16 provider、完整 bundle capability 握手、Inspector personal 确认流、用户触发的原子重建、持久 personal 全库任务、显式服务状态、v009 标准概念持久化、v010 standard Review Queue、显式单图发布与 v011 标准祖先展开已实现；生产公共模型、standard 全库任务、实际 ANE 取证与自动后台训练仍待后续门
> 日期：2026-07-19
...
的祖先并保留来源；生产 provider、standard 全库任务与真实
照片验证仍未接入。
...
1. 标准场景 tracer：fixture 级闭环、v009 ontology/model identity、v010 standard prediction/Review Queue、显式 Inspector 发布与 v011 祖先 DAG 展开已完成；standard 全库任务、生产公共模型 revision、许可证和校准仍待批准；
...
Core ML Xcode 接线、SigLIP2 下载、Ollama 模型下载、生产 standard 模型、standard 全库任务
