...
> 状态：标准/个人 suggestion HTTP、双轨服务启动、个人训练 CLI、DINOv2 Core ML FP16 provider、完整 personal bundle/standard package capability 握手、Inspector personal 确认流、用户触发的原子重建、cache-only 自动个人重训、personal/standard 持久全库任务、显式服务状态、v009 标准概念持久化、v010 standard Review Queue、显式单图发布、v011 标准祖先展开、生产准入 verifier、固定 DINOv2 ANE/resource 取证与独立服务进程端到端 ready 已实现；实际生产标准模型评测/接入仍待后续门
> 日期：2026-07-20
> 开工基线：`main@14c3890b8c83c87a6f603c8900dfaabb3179ea6c`
> 范围：独立模型训练、部署与推理模块，以及可选的当前目录库确认、用户触发训练和 App 运行期 cache-only 自动重训接线；不包含自动模型安装、自动服务启动或生产准确率承诺
...
6. Swift optional client：loopback transport、双轨严格解码、standard capability 全身份门、sandbox/ATS、显式 Inspector standard
   ...
   显式刷新 `GET /v1/health` 的 ready/degraded/unavailable 状态已完成；后台只允许使用既有缓存自动重训，
   不自动启动服务、创建标签、读取图片或下载模型；
