...
> 状态：标准/个人 suggestion HTTP 与个人训练 CLI 已实现；生产公共模型、Core ML 和 App 接线仍待后续门
> 日期：2026-07-18
...
截至 `b92a01b`，门 1～7 已在公开合成 fixture 或合成 embedding 层关闭；个人 bundle 加载同时校验
catalog、bundle revision、DINO encoder、标签词表、policy 和权重身份，personal HTTP 只返回 bundle
内已有 tag 且固定为 `suggested`。门 8 当前只证明模块缺失时
...
3. `/v1/suggestions`：standard 与 personal 双轨、稳定错误及禁止跨轨回退已完成；
