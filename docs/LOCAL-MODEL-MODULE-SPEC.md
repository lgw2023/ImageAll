...
> 状态：Python/HTTP 开发验证链、DINOv2 Core ML 转换/资源证据及既有双轨实验契约已实现；正式部署方向已切换为 App 容器内 Swift/Core ML，固定 artifact tracer 与 App 内启用选择/持久化/状态呈现已实现；Places365 官方权重字节已复核，许可、公开数据和生产准入仍待后续门
> 日期：2026-07-20
...
浏览、人工标签和既有 Feature Print 闭环隔离。下一条推荐 tracer 是 App 容器内版本化 embedding
cache，必须把 preprocessing 与 postprocessing revision、向量元素语义和内容 revision 纳入身份，且先
限定显式单图/程序生成输入，不提前启动全库扫描。
...
| DINOv2-small | 个人标签冻结 embedding | 固定 revision 的 PyTorch/Core ML 开发验证、FP16 artifact、App 内 Swift 加载/embedding tracer 与启用选择已实现；App manifest 固定 source/compiled SHA 与转换工具版本；App 内 embedding cache 和个人线性 head 待后续切片 |
