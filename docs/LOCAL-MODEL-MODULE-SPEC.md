...
> 状态：Python/HTTP 开发验证链、DINOv2 Core ML 转换/资源证据及既有双轨实验契约已实现；正式部署方向已切换为 App 容器内 Swift/Core ML，固定 artifact tracer、App 内启用选择及版本化 embedding cache 已实现；Places365 官方权重字节已复核，许可、公开数据和生产准入仍待后续门
> 日期：2026-07-20
...
SQLite、Review Queue 或个人模型。下一条推荐 tracer 是缓存资源与生命周期窄门，之后再进入 App 内
DINO encoder + Swift/Accelerate 轻量线性 head。
...
| DINOv2-small | 个人标签冻结 embedding | 固定 revision 的 PyTorch/Core ML 开发验证、FP16 artifact、App 内 Swift 加载/启用选择/版本化 embedding cache 已实现；App manifest 固定 source/compiled SHA、转换工具版本与输出语义；缓存资源生命周期门和个人线性 head 待后续切片 |
