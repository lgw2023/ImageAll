...
> 状态：标准/个人 suggestion HTTP、双轨服务启动、个人训练 CLI、DINOv2 Core ML FP16 provider、完整 bundle capability 握手、Inspector personal 确认流与用户触发的原子重建已实现；生产公共模型与自动后台训练仍待后续门
> 日期：2026-07-19
...
生产照片准确率结论或自动后台训练。服务 CLI 可通过 `--provider dinov2 --personal-bundle <path>`，或通过
...
向量和向量 SHA-256；禁止保存原图、base64、路径、bookmark 或 image bytes。当前 Swift client 尚未
编码 `cache_key`，因此这是已交付但尚未由 App 消费的可选后端能力。
...
2. 个人训练：独立 CLI 与 App 用户触发的版本化 embedding/decision 快照、同步重建、原子发布和热重载
   已完成；后端版本化 embedding 持久缓存已完成，Swift 请求采用仍待独立切片；自动后台重训与持久
   任务进度明确延后；
...
未读取 `user/`、`/Volumes/HDD2` 或 `.photoslibrary`，未 push。Swift client 采用缓存键仍未实现。
