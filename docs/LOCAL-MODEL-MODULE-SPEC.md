...
> 状态：Python/HTTP 开发验证链、DINOv2 Core ML 转换/资源证据及既有双轨实验契约已实现；正式部署方向已切换为 App 容器内 Swift/Core ML，首个 App 内固定 artifact tracer 待实现；Places365 官方权重字节已复核，许可、公开数据和生产准入仍待后续门
> 日期：2026-07-20
...
第一切片停止于 Swift 公开接口测试、程序生成图片真实 embedding smoke 和 App build；不修改 SQLite
...
| DINOv2-small | 个人标签冻结 embedding | 固定 revision 的 PyTorch/Core ML 开发验证与 FP16 artifact tracer 已实现；App 内接线待当前切片 |
...
6. 旧 Swift loopback client：其严格解码和身份门仅作为历史实验资产保留，不再是正式部署路径；当前
   新门为 App 内固定 Core ML artifact 的 manifest/SHA/identity 校验、有限 embedding 和无模型降级；
7. 再独立评估 SigLIP2、区域证据、FastViT student 和可选 Ollama VLM adapter。

App 内 Core ML Xcode 接线、SigLIP2 下载、Ollama 模型下载、生产 standard 模型
