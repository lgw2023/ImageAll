...
第九条当前选中单资产 cache 填充切片已完成：

```text
用户显式选择恰好一个资产并点击准备特征
→ 共享激活 actor 先确认固定 App Core ML service ready
→ 既有受控 preview 只读取当前资产与精确 content revision
→ 完整模型 identity 下写入一个版本化 embedding 并 cache-only 回读复核
→ 未启用、模型不可用、iCloud-only 或持久化失败时独立安全提示
```

自动测试只使用程序生成 PNG，得到 384 维有限且身份匹配的 embedding；该动作不扫描图库、不批量预热、
不接后台任务、Review Queue 或个人建议展示。现在一个个人标签的 `2 accepted + 2 rejected` 真实人工小批
标注才开始有验证价值，但 HDD2/PhotoKit 读取、受保护图库挂载时启动 production App、App 容器输出与
停止位置仍需项目所有者针对该次 runbook 明确授权。
...
2. 个人训练：独立 CLI 与 App 用户触发的版本化 embedding/decision 快照、同步重建、原子发布和热重载
   ...
   生产事实/cache-only adapter、共享模型激活实例和显式重建动作已由 `173a117` 关闭，当前选中单资产
   显式 cache 填充已由 `916422c` 关闭；下一门是取得精确只读授权后的单标签真实 `2 + 2` 人工小批，
   不恢复后台自动重训或 loopback 用户依赖；
...
单标签真实 `2 + 2` 人工验证、SigLIP2 下载、Ollama 模型下载和生产 standard 模型都必须按独立切片
重新冻结范围和验收门；真实照片切片还必须单独取得来源、动作、只读证明、输出位置与停止位置授权。
