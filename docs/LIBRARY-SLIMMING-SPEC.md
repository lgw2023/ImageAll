...
- 回收动作的成功、部分失败或失败摘要必须独立于后台分析进度持续可见；分析进度不得遮盖或覆盖
  回收结果。
- 回收请求提交后，所选卡片必须在下一次主线程刷新时从活动簇中乐观隐藏，并以独立状态条显示
  「后台安全转移中」；该状态是已接收/处理中，不能提前表述为成功。失败资产按操作前的簇顺序、选择和
  锚点自动放回，成功资产才进入持久隐藏。普通批量按实际终态持续更新完成数。
- 文件夹物理回收持有短期交互 I/O 优先权：写者优先闸门先阻止新的后台来源读取，再等待当前受保护
...
- 状态条至少区分「等待后台磁盘任务、准备、复制、同步目标、校验目标、复核来源、移除来源、同步
  源目录、等待 PhotoKit、完成」，并显示文件资产已复制字节/总字节；阶段和整批日志只记录聚合数量与
  耗时，不记录路径、文件名、asset ID 或 Photos local identifier。
