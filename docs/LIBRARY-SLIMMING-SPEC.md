...
| LS-P25 | 簇内选中照片后可按 Space 打开接近全工作区的大图预览；预览按原比例适配且不超出可用区域，←/→ 在当前簇切换，Space/Esc 退出，Delete 退出并对当前可见照片进入既有快速删除确认 | **所有者批准（2026-08-04）** |
...
当前选择。Delete 先退出预览，再对屏幕上当前这一张打开现有“快速删除并释放空间”确认；不得因
键盘入口绕过永久删除确认或 Photos 系统回收语义。
...
| 大图预览 | Space 从最后聚焦项打开并收敛单选；←/→ 簇内有界移动并重置 100%；Space/Esc 退出保留选择；Delete 只锁定当前可见照片并进入既有快速删除确认；滚轮缩放限制 100%–800%；画布裁剪放大内容；仅溢出轴允许左键拖动且平移不露空白 |
