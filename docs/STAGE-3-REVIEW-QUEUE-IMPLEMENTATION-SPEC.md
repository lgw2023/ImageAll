...
>
> 日期：2026-07-16；标签概览排布修订：2026-08-04
>
...
- 标签概览复用主侧栏持久化的标签分组、分组顺序、组内标签顺序和折叠状态；审核状态不再另建一套排序。
...
概览一级结构与主侧栏一致：按持久化 `tag_group.sort_order` 分组，组内严格复用用户维护的
标签顺序，分组折叠状态也共享。运行状态和待审核条数作为卡片内的视觉提示，不再改变标签
位置；这条 2026-08-04 决策替代此前按任务状态和待审核条数重新排序的规则。

标签卡采用高密度排布：标签名为首要信息并使用不小于正文的强调字号；样本数、状态和待审
来源紧邻名称展示；三轨门槛与生成动作默认折叠，展开后仍保留全部编辑和任务控制能力。
...
第 7 节是本规格批准时的停止位置，保留为历史范围说明；其后的 PhotoKit 扩展已经交付。冻结
evaluation cohort、离线 metrics 与面向用户的强度文案继续延期；三个代表性标签的运行校准见下节。

### 8.1 三个代表性标签运行校准

2026-07-17 使用第 8 节同一 Apple Development 签名 App、同一 active Apple Photos Source 和 9480 个
可用静态资产，只通过 PhotoKit 与应用 UI 对“人像”“美食”“风景”完成两轮“标样本—出建议—纠正—
重新排序”。每轮只运行到首批结果足以复核后暂停并取消，已发布建议与人工事实保留；既有 `Stage4Y 验收`
任务继续暂停。实验性的“动漫”标签因正例 Feature Print 不可用而取消并归档，历史人工事实未删除。

| 标签 | revision 1 顶部 6 项 | revision 2 顶部 6 项 | revision 2 首批待审核 | 复核后人工事实 |
|---|---:|---:|---:|---:|
| 人像 | 6/6 属于人像 | 6/6 属于人像，score `0.404538...0.452069` | 1855 | 14 accepted / 2 rejected |
| 美食 | 0/6，均为人物误报 | 0/6，转为动漫、游戏或文档误报，score `0.024935...0.077354` | 24 | 2 accepted / 14 rejected |
| 风景 | 6/6 属于风景 | 6/6 属于风景，score `0.584191...0.899083` | 1111 | 14 accepted / 2 rejected |

校准结论不是“三个标签都达到通用精度”。它证明人像和风景在 `2 + 2` 起步样本下即可形成稳定顶部
排序，也证明给美食加入 6 个高分人物硬负例只把误报迁移到另一簇，不能用提高一个全局阈值替代正例
代表性。三个标签的 score 范围也明显不可直接比较。因此冻结以下产品决策：

- 保留最少 2 个正例和 2 个反例的生成硬门，不阻塞已经有效的标签；
- 概览非阻塞建议正反样本各至少 4 张并尽量覆盖不同内容，由 `8a1b3aa` 实现；
