...
> 状态：Authorized / Blocked at runtime safety gate；标签缺陷已关闭，等待受保护图库静默后重启
> 日期：2026-07-20
...
修复后的重启安全门进行了两次新的聚合 FSEvents 观察。第一次在 ImageAll 启动前已记录 37 个写类事件，
短暂启动后累计 275；第二次没有启动 ImageAll，仅观察 10 秒仍累计 90。事件先于 App 且无法按进程归因，
因此不能把它们算作 ImageAll 写入，也不能据此声明源端零写通过。两次监控与短暂启动的 App 均已停止，
没有人工选择、标签或特征准备动作。恢复条件是由项目所有者关闭 Photos 或等待其系统维护结束，再以一份
新报告先证明至少 10 秒零事件基线；只有基线为零才重新启动 ImageAll。
...
| 失败隔离 | 待人工完成 |
| 源端写入 | 重启前已有非 ImageAll 可归因写类事件；安全门未通过，须在静默基线后重新证明 |
| 停止位置 | App 与两次新监控均已停止；零人工样本，等待项目所有者关闭 Photos/系统维护静默 |
