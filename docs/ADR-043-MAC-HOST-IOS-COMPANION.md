...
| **R3** | 正式配对 + TLS + 持久幂等 + WebSocket + 预览/审核/任务远程面（见 ADR-044） | 可配对、HTTPS 会话、事件推送与完整辅助闭环 |
| **R4** | 异地中继 / 产品化 Relay | 另开 ADR |

### 6. 版本与兼容

- App 版本可独立；协议用 `protocolVersion` + `capabilities`。
- 同仓原子改协议；不强制 Mac/iOS 同号发布。
- 在仍接受旧协议版本时，给既有 DTO 增加字段必须提供缺省解码语义；标签分组缺失时归入稳定的
  「物品与其他」分组，审核媒体类型缺失时按照片处理，不能让新客户端因旧 Host 少字段而解码失败。
