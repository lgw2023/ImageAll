...
首个性能基准使用三档目录库：1 万、10 万和 100 万条合成元数据；图片解码基准另用可公开分发的小型真实格式集，避免把合成行查询与真实图像 I/O 混成一个指标。当前三档查询基线、10 万与 100 万可移植导出门，以及 100 个有效 JPEG 可丢弃 fixture 的“扫描—查询—缩略图生成—缓存命中”图片 I/O 基线已经关闭；真实 SwiftUI 交互与滚动、混合真实格式分布及端到端大容量图片 I/O 仍是独立验收门，详细证据与停止位置见阶段 4 实施规格第 6 至 11 节。
...
导出、10,000 对象缓存清理压力门、发布前隐私/故障回归和 100 个有效 JPEG 可丢弃 fixture 的端到端
图片 I/O 基线已经关闭，真实 macOS 权限、真实 SwiftUI 交互与滚动、混合真实格式分布及端到端大容量
图片 I/O 门仍待完成。
