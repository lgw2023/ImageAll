...
| `standard_model_revision`（规划） | `ontology_id`, `ontology_revision`, `revision`, `provider`, `weights_hash`, `preprocessing_revision`, `policy_revision`, `created_at` | 一组标准概念共用的公共模型版本 |
...
- 个人标签的 `normalized_name` 在个人命名空间唯一；标准标签以 `ontology_id + concept_id + ontology_revision` 唯一，本地别名不改变概念身份；
...
状态：独立双轨 tracer、HTTP、CLI 装载、Swift client transport、Inspector 标准/personal 单图流、DINOv2 Core ML FP16 导出/HTTP provider、用户触发的 personal 快照重建与原子 bundle 生命周期，以及 personal 全库建议进入现有 Review Queue 的首条 App tracer 已实现；持久 personal 扫描任务、生产公共模型与自动后台训练仍待后续切片。独立 loopback 服务、
...
job/progress/cancel 状态机。用户显式触发的 personal 全库建议现可写入统一 Review Queue，但仍只在
当前 App 会话内串行运行；持久 checkpoint/pause/resume/cancel、自动后台重训、标准概念持久化
schema/Review Queue、Xcode compute plan 与生产模型安装继续分别验收。
