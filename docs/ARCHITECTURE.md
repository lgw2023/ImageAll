...
当前阶段只要求上述闭环可运行、可恢复且不覆盖人工事实，不把现有照片的自动标签准确率、
建议覆盖率或预设标签完整性作为 MVP 验收门。内置算法是本地轻量基线，用于验证产品与工程链路；
面向现有相册的通用语义标签预测计划在后续阶段评估并接入合适的 Hugging Face 模型。
...
- 应用可以提供一组小而通用的标签预设，但预设只创建缺失的普通 `Tag`，不得自动写入
  `asset_tag_decision`、生成预测或启动分析任务；已有同名标签按统一标准化规则幂等跳过；
- 预设标签不进入数据库枚举或约束，安装后与用户自建标签完全相同，可以重命名和归档；
...
- 任一模型输出都只能写入可重建 prediction，并经 Review Queue 由用户确认或拒绝，不能直接
  变成人工标签事实。
...
所有推理实现通过可替换 `PersonalizationEngine` / feature provider 边界接入，并至少记录 provider、
模型 revision、权重校验标识、预处理 revision 和标签词表 revision。切换 provider 或任一 revision 时，
旧 prediction 失效并可重建，人工决定和用户自建标签保持不变；不同 provider 的原始 score 不得直接
比较或沿用同一强度文案。模型下载、离线安装、许可证展示、存储配额、签名校验与沙盒路径须在该
阶段单独形成规格，当前 MVP 不提前冻结。

在不改领域模型的前提下，算法可以按价值逐步演进：

1. 输出数值向量的 Core ML embedding 模型；
2. 每标签逻辑回归或线性分类器；
3. 本地近似最近邻索引；
4. 从 Hugging Face 选择并本地化通用图像分类、多标签或视觉 embedding 模型；
5. 标签体系稳定后训练正式多标签 Core ML 模型；
6. 再按需加入 OCR、日期、位置和相册关系等多模态特征。

这些演进能力都不进入当前 MVP；当前基线准确率或样本覆盖不足本身不构成提前引入它们的门槛。
...
状态：First tracer slice implemented。独立 loopback 服务、固定 revision 的 DINOv2-small、MPS
线性多标签 head、锁定依赖、真实模型 HTTP smoke 与无模块 App build 已由 `058a161` 交付。
该阶段把 Hugging Face 视觉 encoder、线性多标签训练、
