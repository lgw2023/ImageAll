...
| `training_run`（规划，见 ADR-038） | `id`, `method`, `state`, 时间戳, `catalog_scope_id`, `job_id?`, `sample_summary_json`, `config_json`, `metrics_json`, artifact 指针, `result_summary_json` | 每次训练的可审计工程记录；三方法共用 |
| `personal_suggestion_model` | 规划改为按 `method` 多槽（`personalCentroid` / `personalAdamW`），废止全局 singleton 互顶；见 ADR-039 | 当前已发布个人头 capability；每槽独立 published |
| `personal_prediction` | 规划 PK 含 `method`，与对应槽级联 | 个人轨可重建待审建议；质心与 AdamW 并存 |
...
- ADR-039 生效后：`personal_suggestion_model` 按 method 至多一行；`personal_prediction(method, asset_id, tag_id, content_revision)` 唯一；质心与 AdamW 预测不得互相级联删除；
- ADR-038 生效后：`training_run` 为追加型历史，成功 Run 不因后续训练删除；
...
激活会互相 `DELETE` 覆盖；ADR-039 改为按 method 多槽并存，激活只替换本槽预测。ADR-038 要求每次
训练写入 `training_run`（含 AdamW 过程指标），并提供独立「训练工程」工作区。
...
准入门见 [`STANDARD-MODEL-ADMISSION-SPEC.md`](./STANDARD-MODEL-ADMISSION-SPEC.md)。当前 Places365
...
CC BY，官方项目页还把 CNN/数据用途描述为 academic research/education；在许可义务和公开数据
manifest 完整前保持 `research`，不得替换现有 fixture。`1699138` 已交付不下载模型的
...
| ADR-025 | 标准标签使用版本化 ontology DAG 和独立公共模型生命周期 | ontology/model identity、祖先展开与生产准入 verifier 已实现；Places365 候选仍为 research | 一个概念可属于多个父类；model、mapping、policy 与 ontology 分别版本化，冲突安装不迁移人工决定；来源、许可、公开评测、转换和资源证据全部通过前不得生成生产包 |
...
7. 训练工程导航壳层：侧栏一级「训练工程」整页切换（规格暂定）还是顶栏图库/训练模式切换；以及 Run 列表是否保持统一列表（规格暂定），见 `TRAINING-WORKSPACE-SPEC.md` TW-P6/TW-P7；
8. ADR-033 文档中的每标签 `2 + 2` 与当前 App 质心/AdamW「至少 2 正例」实现是否收敛为同一硬门。
