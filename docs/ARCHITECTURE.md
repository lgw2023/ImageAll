...
状态：Slice A-G implementation and targeted automated acceptance complete；发布前真实图库人工 smoke 与
全量性能绿色门仍开放。连接、授权、单 active Photos 来源、持久化分批元数据对账、统一网格/来源筛选、
...
保留历史事实并新建 active Source 的重绑定体验已完成。当前收口 HEAD `dabb86e` 的相关回归 99/99、Apple
Development entitlement 1/1 与 arm64 Debug 严格验签 build 均通过；unsigned 全量 808 项中 807 通过，
唯一失败是既有百万资产查询在全量并发环境以 10.625 秒超过 5 秒阈值；隔离复跑为 4.929 秒，其中 search
占 4.858 秒，性能余量仍不足。实施顺序
与验收边界见
