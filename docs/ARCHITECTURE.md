...
状态：Completed。Gate 0 与切片 1–6 已通过，批准实现为 `892f4e2`；详细契约见 [STAGE-0-IMPLEMENTATION-SPEC.md](./STAGE-0-IMPLEMENTATION-SPEC.md)，验收记录见 [STAGE-0-EVIDENCE.md](./STAGE-0-EVIDENCE.md)。阶段 1 正在实施，切片 1 已通过，批准实现为 `81c35e7`。
...
状态：Implementation in progress。切片 1（目录库查询与人工标签事务）已通过，批准实现为
`81c35e7`；切片 2 已获准仅实施授权引擎与 AppKit 选择器适配，产品“连接文件夹…”入口仍留到切片 6。后端只读资产流、generation/FSEvents 与安全验收边界见
