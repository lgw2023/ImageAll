...
1,021 个 Active ANE inference 区间。依赖初始化 1.6017 秒单列；这不是端到端服务 ready 结论。
...
plan/resource 报告及固定 DINOv2 的实际 Neural Engine、峰值 RSS/热状态和预编译安装已经关闭；
App 运行期的 cache-only 自动个人重训也已关闭；端到端服务启动、生产标准包接线和真实照片只读
smoke 仍为后续独立验收门。
...
revision 门；cache-only 自动个人重训已经验收，端到端服务启动仍待独立验收，未来 production standard artifact 仍须重做
