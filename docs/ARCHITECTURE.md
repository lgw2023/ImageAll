...
内容多样样本；冻结 evaluation cohort、离线 metrics 和强度文案继续延期。
...
| ADR-005 | MVP 用 Vision Feature Print 做少样本建议 | 已决定并完成三标签运行校准 | 人像与风景顶部排序稳定，美食暴露样本代表性限制；保留 `2 + 2` 硬门、`4 + 4` 多样样本引导与内部 `score > 0`，不宣称跨标签概率或固定强度 |
