...
smoke 仍为后续独立验收门。当前第一条产品切片改为 App 内固定 DINOv2 Core ML artifact：Swift 公开
接口必须以程序生成图片返回 384 维有限且完整身份匹配的 embedding；模型未启用、缺失、损坏或身份
不匹配时稳定降级，并且不影响浏览与人工标签。
...
| ADR-030 | 正式 App 在自身容器内校验并由 Swift 直接加载 Core ML；不启动 Python/HTTP/helper/XPC | 已决定，首个 App 内 tracer 实施中 | 新用户只安装 App；模型缺失/损坏/未启用安全降级；首发小模型可随包，较大模型以后设计受校验按需下载；Developer ID/MAS 不阻塞接线 |
