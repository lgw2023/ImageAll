...
capability；旧 bundle 和人工事实不因后台失败而改变。该能力只在 App 运行且可选 loopback runtime
存在时启用，不自动启动服务、下载模型或读取 iCloud-only/受保护真实照片。
...
ResNet18 上游代码可固定，但预训练权重仅声明未标版本的 CC BY，官方 HTTP artifact 也没有发布
SHA-256，官方项目页还把 CNN/数据用途描述为 academic research/education；在官方字节复核、许可
义务和公开数据 manifest 完整前保持 `research`，不得替换现有 fixture。`1699138` 已交付不下载模型的
...
| ADR-029 | 当前只验收可选 loopback 服务的独立进程生命周期，不让 App 启动开发 venv | 独立服务门已实现；App 托管待分发决策 | 工作区路径和 Python venv 不能成为签名 App 依赖；未来若要求 App 自动托管，必须先冻结 signed helper/XPC、安装升级、签名校验和退出恢复契约 |
