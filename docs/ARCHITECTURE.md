...
除 ADR-044 中由用户从图库瘦身、普通图库 Inspector 或单图查看显式确认的受限回收/清理机制外，应用不搬运、不修改原图。ADR-055 另允许只通过公开 PhotoKit 双向同步资产红心元数据，不读取或改写原图字节。应用在自己的 SQLite 目录库中保存资产索引、独立红心事实、标签定义、人工确认/拒绝记录、视觉特征版本和模型预测。标签能力分为两条并行轨道：公开世界知识对应版本化的标准标签及公共模型，可以在用户尚未积累标注时提供带来源的自动标签；个人关系、用途和偏好对应用户自定义标签，只能随用户正负反馈逐步学习。人工决定永远优先，任何模型结果都不得伪装或覆盖人工事实。
...
首版不做云端服务、不写回 Apple Photos 关键词、不训练端到端神经网络。原图在索引/分析/导出路径上保持只读；**唯一**允许移动/删除原图的产品机制是 ADR-044 的用户确认回收/清理流程，可从图库瘦身、普通图库 Inspector 或单图查看进入，不包含静默去重或其它功能顺带删图。红心同步是 ADR-055 批准的独立 PhotoKit 元数据写回，不扩大原图写权限。
...
- 不托管或同步原图；
- 不依赖账号、服务器或互联网才能使用；
- 除 ADR-044 用户从图库瘦身、普通图库 Inspector 或单图查看显式确认的文件夹回收/清理或 Photos 系统软删除机制外，不修改、移动或删除来源中的原图；
- 不直接读取 `.photoslibrary` 包内部文件；
- 不向 Apple Photos 写入任意关键词；仅 ADR-044 批准的图库瘦身、普通图库 Inspector 或单图查看删除入口可在用户确认后经公开 PhotoKit 把资产移入系统「最近删除」，恢复与永久删除由 macOS「照片」App 管理；
...
1. **原图默认只读**：任何索引、分析或导出失败都不能损坏用户资产。唯一例外是 ADR-044 受限机制中经用户从图库瘦身、普通图库 Inspector 或单图查看显式确认后，将文件夹资产移入 ImageAll 回收站或快速永久删除，或把 Photos 资产经公开 PhotoKit 移入系统「最近删除」；Photos 的恢复与永久删除仍由 macOS「照片」App 管理。该例外不得被其它子系统复用。
...
10. **模型进程内本地化**：正式 App 只由 Swift 直接加载已校验 Core ML artifact；Python/uv、loopback
    HTTP、helper/XPC 仅可作为仓库内开发工具或历史证据，不进入普通用户运行路径。
...
MVP 支持的格式以 Image I/O 能稳定解码的静态图片为主，并按 ADR-041 纳入富士/Adobe 等 camera-raw、JPEG 2000、静态 GIF、SVG、单页 PDF 与单页 PDF 兼容 Illustrator AI。解码级联为 Image I/O → 矢量文档验证/栅格化 → Core Image RAW → LibRaw 兜底；矢量原件保持只读，派生缓存仍为 JPEG/PNG。多页 PDF/AI、传统非 PDF AI、EPS、SVGZ 与动画/非 RAW 多帧仍不进入 `available`。文件夹来源不识别 Live Photo 组合；PhotoKit 来源把 Live Photo 的静态主图作为普通支持图片读取，不读取其视频伴随资源。
...
历史 loopback DINO tracer 已实现用户触发的最小重建闭环：App 只选择 current catalog 中 active 且满足
...
当前已确认的完整 capability 与可重建建议。历史实现把 personal 表做成全局 singleton，质心与 AdamW
激活会互相 `DELETE` 覆盖；当前已按 ADR-039 改为 method 多槽并存，激活只替换本槽预测。ADR-038
已要求并实现每次训练写入 `training_run`（含 AdamW 过程指标）以及独立「训练工程」工作区。
...
标签词表 revision 和 `asset_id + content_revision` embedding cache key，不含图像、locator、路径、
bookmark 或 bytes。handler 在调用服务前后都复核快照，过期任务直接完成；Backend 的
`/v1/personal/rebuild-cached` 只读取已由显式重建填充的版本化 embedding 缓存，不允许补读照片或回退
在线 embedding。缓存 miss 终止该任务并保留手动重建入口，服务不可用和 bundle CAS 冲突按既有队列
重试。候选 bundle 完整加载、capability 二次确认且快照仍当前后，App 才原子激活新的 personal
capability；旧 bundle 和人工事实不因后台失败而改变。该历史能力只在 App 运行且开发用 loopback
runtime 存在时启用，不自动启动服务、下载模型或读取 iCloud-only/受保护真实照片；正式部署不再采用
该运行时。当前方向是在 App 容器内由 Swift 直接加载同一已校验 DINO Core ML encoder，并在 App 内
训练/推理轻量线性 head，沿用 catalog scope、标签词表、人工快照和 fail-closed 身份门。
...
| 候选 | 轨道与职责 | 当前状态 |
|---|---|---|
| Places365 ResNet18 | 标准轨道的场景分类与 indoor/outdoor 等公共属性（历史研究候选） | **2026-07-24 已产品拒绝作生产包**；官方字节已核，许可/再分发不清；归档 `research`，不得进 App |
| DINOv2-small | 个人轨道的冻结视觉 embedding，供近邻或轻量线性分类器使用 | 独立模块 tracer 已实现 |
| SigLIP2 B/32 256 | 标准概念候选发现、开放词表评测和 teacher 实验 | 固定 revision 的官方仓库约 1.55 GB，只读资源预筛拒绝直接首包路径；保留 teacher 研究候选 |
...
开发验证用 Python DINO 模块使用独立 managed store 承载历史 bundle 文件生命周期，不把权重写入 App
...
该开发进程的当前身份来源，`tag_model` 表的既有 Feature Print 生命周期不被静默改写。正式 App 的
权威模型状态改为 App 容器内 manifest、artifact SHA-256 与 active revision；不从 HTTP capability 获取。
...
必须转换为经过准入的 Core ML artifact 并由 Swift 直接加载，目录库、Review Queue 和人工标签契约不得
绑定仓库名、模型名或某个推理框架。
...
比较或沿用同一强度文案。首发小模型可随 App 分发；较大可选模型的按需下载、许可证展示、存储配额、
签名校验与沙盒路径须在后续单独形成规格，当前 MVP 不提前冻结。
...
- 除 ADR-044 批准并由用户从图库瘦身、普通图库 Inspector 或单图查看显式确认的文件夹回收/清理与 Photos 系统软删除入口外，不提供任何原图删除、移动或覆盖入口；
...
派生图片缓存不设置应用层总量配额；可写容量由目标卷的实时资源事实决定，同时保留 `max(5 GiB, 卷容量的 5%)` 作为不可突破的安全余量。只有实际可用容量无法容纳新对象与安全余量时，才按 LRU 回收可重建缓存；空间仍不足时停止新的派生写入，不删除事实数据。交互式网格与 Inspector 可以在已完成来源校验和内存渲染后返回不落盘的临时派生图；该降级不得创建 cache entry、object 或 staging 文件，后台持久化请求仍返回空间不足。批量特征任务开始前按已观察平均条目大小估算本次增量占用并展示；缓存淘汰只删除数据库可重建且没有任务租约的文件。固定并发值与原 20 GiB 契约的取代依据见 ADR-058。
...
状态：Implementation complete；real-data acceptance open。切片 1～4 已完成目录查询、授权、流式对账和派生图缓存；加速纵切片 A～F、W 已交付“连接—扫描—浏览—人工标签—搜索筛选—单图键盘浏览—状态/格式筛选—稳定排序—来源重扫/重授权/停用—标签重命名/归档—文件夹变化自动重扫”，当前批准实现为 `eb538d6`。FSEvents 与活动控制已关闭，非关键验证继续延后。加速顺序见
...
共享 App 模型激活实例、工具栏显式原生重建和当前选中单资产显式 App 内 DINO embedding cache 已经
关闭。下一门是一个个人标签的真实 `2 accepted + 2 rejected` 人工小批验证；它必须先取得 HDD2/PhotoKit
来源、production App 启动、只读动作、App 容器输出和停止位置的精确授权，不扩大为全库扫描、后台预热、
iCloud 下载或个人建议展示。

状态：独立双轨 tracer、HTTP、CLI 装载、Swift client transport、Inspector 标准/personal 单图流、DINOv2 Core ML FP16 导出/HTTP provider、用户触发的 personal 快照重建与原子 bundle 生命周期、cache-only 自动个人重训、personal/standard 持久全库任务、完整 standard package capability 握手、显式本地服务状态、v009 标准概念持久化、v010 standard Review Queue、显式单图发布与 v011 标准祖先展开已实现；生产公共模型准入规格已冻结，Places365 ResNet18 已于 2026-07-24 正式拒绝作生产包并归档为 `research`，固定 DINOv2 的实际 ANE、预编译安装、峰值 RSS、热状态与独立服务进程端到端 ready 取证已关闭，下一生产标准场景包须另选候选。独立 loopback 服务、
...
standard package capability 握手由 `d157487` 交付：Backend 只从已完成 checksum/package 校验的 engine
公布 pack、manifest、ontology、provider/model、mapping、policy 与 weights 完整身份；App 单图与全库
入口先对照内置批准 fixture 全量核准，之后才安装 ontology、读取图片或入队。unavailable、畸形和
identity mismatch 均在副作用前 fail closed，CompositionRoot 不再持有硬编码 standard target。
cache-only personal 重建 endpoint 与 App 运行期 30 秒防抖持久任务分别由 `8946846`、`42b6423`
交付；自动路径只发送人工决定与 embedding cache key，过期快照跳过，cache miss 保留手动重建，服务
与 CAS 故障进入既有 retryable 队列，后台不读取图片、不启动服务。
独立 loopback 服务启动验收器由 `7e0874b` 交付：目标用 argv 启动而不经过 shell，只允许显式
`127.0.0.1`，严格核对 health/capability 并用程序生成 PNG 走 embedding 或 standard HTTP，证据不含
完整 argv/路径，任何成功或失败路径都确认子进程停止。固定 DINOv2 的实际 Core ML 服务已完成
“启动 → ready → 384 维有限 embedding → SIGTERM 退出”，见
[`coreml-loopback-service-startup-dinov2-small-2026-07-20.json`](./evidence/coreml-loopback-service-startup-dinov2-small-2026-07-20.json)。
生产 standard 模型的候选身份、许可证、公开评测、suggested-only 校准、Core ML parity、资源和 App
准入门见 [`STANDARD-MODEL-ADMISSION-SPEC.md`](./STANDARD-MODEL-ADMISSION-SPEC.md)。Places365
ResNet18 上游代码可固定，MIT 官方 HTTP 实际字节已于 2026-07-20 复核为 `45,506,139` bytes、SHA-256
`2f4759217d470da2b803f8f66cd4488a066406b555a5fb95ee9a4663f9f05588`。但预训练权重仅声明未标版本的
CC BY，官方项目页还把 CNN/数据用途描述为 academic research/education；项目所有者于 2026-07-24
**正式拒绝**其作为生产 standard 包候选（ADR-042），归档为不可升级的 `research` 证据，不得替换现有
fixture。`1699138` 已交付不下载模型的
严格离线证据 verifier；它只能根据完整报告收敛状态，不能把自报 approved 或占位 hash 当作批准。
...
`7e57484` 新增独立 Instruments 证据 verifier；2026-07-19 的固定 DINOv2 离线运行在 Xcode
Instruments 16.0 `Core ML` 模板中记录到非空 `Apple Neural Engine` / `Active` /
`AneInference` 区间，并与退出码 0、通过的 8 张程序生成输入 benchmark 共同收敛为
[`coreml-ane-dinov2-small-2026-07-19.json`](./evidence/coreml-ane-dinov2-small-2026-07-19.json)。
该证据只证明该固定 artifact 在这次运行中至少一次实际使用 ANE，不外推为全部算子、全部推理或未来
standard 模型的调度结论。原始 trace 会包含本机环境快照，不能入库；仓库只保存 verifier 的去标识
聚合结果。
`f56f62c`、`4d570c2` 与 `b739cfe` 进一步交付 runtime-only 资源 benchmark、原子 schema v2
`.mlmodelc` 安装和带 `id/ref` 解压的资源 trace verifier。预编译 DINOv2 的完整去标识证据见
[`coreml-compiled-runtime-dinov2-small-2026-07-19.json`](./evidence/coreml-compiled-runtime-dinov2-small-2026-07-19.json)：
43,424,525 字节 artifact、0.0873 秒模型冷加载、53,067,776 字节峰值 RSS 增量、1,000 次零失败/零
非有限输出、thermal 全程 nominal、median/p95 为 2.7249/2.8191 ms，并在同一 exit 0 运行中记录
1,021 个 Active ANE inference 区间。依赖初始化 1.6017 秒单列；该资源报告自身不等于服务 ready，
后续独立启动探针已另行关闭服务生命周期门。

历史第一切片没有修改 App target、SQLite schema 或 UI，只用合成图片证明独立 loopback 服务、版本化
embedding 和 MPS 线性 head 训练。Python 侧 Core ML FP16 数值一致性、HTTP provider 与只读 compute
plan/resource 报告及固定 DINOv2 的实际 Neural Engine、峰值 RSS/热状态、预编译安装和独立服务
...
LibraryWorkspace、图库扫描、SQLite 或个人模型。`393e0dc` 进一步交付 App manifest revision 2、完整
preprocessing/postprocessing/向量元素语义身份和 App Caches 内版本化 embedding cache；显式程序生成
输入支持跨实例 exact hit、content revision miss、SHA/NaN 损坏重建、缓存失败实时降级及同实例同 key
...
重载后通过 Accelerate `vDSP` 只返回已知标签的有限正分建议，篡改或身份不匹配 fail closed。
`88b0b2a` 进一步在 App Application Support 下以内容寻址 `.personal-head` 对象和单一 `active.json`
管理候选与 active；候选经 no-follow 落盘重载、artifact SHA、原始字节、catalog 和完整 encoder identity
...
唯一批准 package 精确相等后才可进入单图或全库路径，POST 响应仍由既有第二身份门复核。个人训练 CLI 已验证“DINO embedding + 稀疏人工正负决定 →
...
推理产物并保持 loopback-only。历史 Swift client 现只存在于测试/开发验证 target，只接受
`127.0.0.1` 并再次复核 track 与完整 revision identity；正式 App target 不编译该 client。
...
CPU_ONLY/ALL benchmark 自身仍只记录请求的 compute units，不单独宣称 ANE 调度；上述独立
Instruments 证据仅把固定 DINOv2 的本次实际分配升级为 verified。已验证 artifact 现可通过
...
原子 `active.json` 与热重载，App 随后以 capability 二次确认。该显式动作继续承担缓存 miss 时的照片读取
与恢复入口；成功写入的版本化 embedding 缓存则供 30 秒防抖的持久 cache-only 后台重训任务复用。
用户显式触发的 personal 全库建议现通过统一持久任务写入 Review Queue，
...
revision 门；cache-only 自动个人重训和独立服务启动已经验收，未来 production standard artifact 仍须重做
自身的 ANE 取证，不能复用 DINOv2 结果。Review Overview 还可由
...
| ADR-012 | 阶段 1 只允许 JPEG、PNG、HEIC/HEIF、TIFF、WebP | 已被 ADR-041 在格式边界上取代 | 原决定先覆盖主流静态图片并延后 GIF/RAW/PDF；后续边界以 ADR-041 为准；文件夹不识别 Live Photo 组合，PhotoKit 仅纳入其静态主图 |
| ADR-041 | 扩大静态、RAW 与矢量文档解码 | 已决定 | camera-raw + JPEG 2000 + 静态 GIF + SVG + 单页 PDF/PDF 兼容 AI；多页 PDF/AI、传统非 PDF AI、EPS、SVGZ 仍非 available；权威细节见 `ADR-041-EXPANDED-MEDIA-DECODE.md` |
...
| ADR-023 | Hugging Face 模型先进入独立 loopback 模块，训练用 PyTorch/MPS，App 部署主线用 Core ML，Ollama 仅作可选 VLM adapter | 开发 tracer 已实施；产品运行部分由 ADR-030 取代 | 独立 runtime 仍可用于转换、离线评测和开发验证，但不再作为普通用户依赖 |
...
| ADR-025 | 标准标签使用版本化 ontology DAG 和独立公共模型生命周期 | ontology/model identity、祖先展开与生产准入 verifier 已实现；Places365 已产品拒绝（ADR-042） | 一个概念可属于多个父类；model、mapping、policy 与 ontology 分别版本化，冲突安装不迁移人工决定；来源、许可、公开评测、转换和资源证据全部通过前不得生成生产包 |
...
| ADR-036 | Settings 与个人模型重建共享同一 App Core ML 激活实例，生产重建只读既有精确身份 cache | 已实现 | 避免第二份模型初始化；未启用、损坏或 cache miss 在图片读取前 fail closed，显示独立提示且浏览和人工标签继续可用；新图片读取边界由 ADR-037 收紧 |
...
| ADR-044 | 图库瘦身双轨回收、普通图库显式删除、跨来源相同检测、Photos 原图长期保存与可恢复大库分析 | S0–S8 与普通图库 Inspector/单图删除已交付；授权真实云下载 smoke 已通过（2026-07-27） | 只有用户显式确认才处置；普通图库与瘦身共用强制确认、红心保护和来源分轨；iCloud-only 相同检测可隐式下载原始内容；长期副本默认无限期保留并可手动全量清理 |
...
| ADR-059 | 大图库浏览热路径采用提前分页、排序索引、分层缓存、底层取消与单元格快照 | 已决定并实施（2026-08-16） | 不改变只读与联网边界；合成规模门已通过，真实图库端到端帧率仍需明确授权验证 |
...
7. ADR-033 文档中的每标签 `2 + 2` 与当前 App 质心/AdamW「至少 2 正例」实现是否收敛为同一硬门。
