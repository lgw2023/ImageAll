...
状态：独立双轨 tracer 与 HTTP 已实现；生产模型、Core ML 与 App 接线待后续切片。独立 loopback 服务、
...
DINO 稀疏训练 CLI 与 suggestion HTTP 由 `abc5ef0`、`b92a01b` 交付。
...
`tag_id` 且固定为 `suggested`，身份不匹配 fail closed 且不回退 standard。下一独立模型切片进入
