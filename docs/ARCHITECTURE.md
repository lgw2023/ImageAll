...
缓存预算不是阶段 4 才补的优化，而是首次生成缩略图前的硬约束。MVP 用户配额默认 20 GiB 并允许调整，同时保留目标卷 `max(5 GiB, 卷容量的 5%)` 作为不可突破的安全余量；实际可写额度取用户配额与安全余量约束后的较小值。空间不足时停止新的派生写入，不删除事实数据。批量特征任务开始前按已观察平均条目大小估算本次增量占用并展示；缓存淘汰只删除数据库可重建且没有任务租约的文件。
...
状态：Completed。Gate 0 与切片 1–6 已通过，批准实现为 `892f4e2`；详细契约见 [STAGE-0-IMPLEMENTATION-SPEC.md](./STAGE-0-IMPLEMENTATION-SPEC.md)，验收记录见 [STAGE-0-EVIDENCE.md](./STAGE-0-EVIDENCE.md)。阶段 1 已进入设计，尚未实施。
...
状态：Designing。后端只读资产流、generation/FSEvents 与安全验收边界已形成
[`STAGE-1-BACKEND-ARCHITECTURE-DRAFT.md`](./STAGE-1-BACKEND-ARCHITECTURE-DRAFT.md)；
产品界面方向已整理为
[`STAGE-1-PRODUCT-UI-SPEC-DRAFT.md`](./STAGE-1-PRODUCT-UI-SPEC-DRAFT.md)，等待项目所有者逐项确认。在产品界面决策完成前不生成 Cursor 实施交接单。
...
| ADR-008 | 阶段 0 暂以 macOS 15.0、Apple Silicon 开发 | 暂定，阶段 1 前冻结 | 允许工程开工；是否支持 Intel 和更低版本仍是产品决策 |
| ADR-009 | 文件资产采用保守身份判定 | 已决定 | 宁可要求人工重连，也不把旧标签继承给新文件 |
| ADR-010 | 首个 migration 只建立核心事实与任务表 | 已决定 | 避免为尚未实现的特征、模型和预测预建 schema |

## 20. 尚待确认的问题

以下问题不阻塞阶段 0，但必须在相应阶段前决定：

1. 暂定 macOS 15.0 是否成为首个正式最低版本，以及是否支持 Intel；
2. 仅自用签名分发、Developer ID，还是 Mac App Store；
3. 首版是否只索引和浏览 RAW 元数据/缩略图；RAW 特征与分类仍明确不进入 MVP；
4. iCloud 分析图片默认尺寸和下载配额；
5. 每个标签开始建议所需的最少正例/反例数量；
6. 用户数据导出格式采用单一 JSON、JSONL，还是 SQLite 快照加 manifest；
7. 是否需要在 MVP 后提供可选 XMP sidecar 写入；
8. 真实目录库的大致图片数量、主要格式和所在存储类型，用于冻结性能基线。
