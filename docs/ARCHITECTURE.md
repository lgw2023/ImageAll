...
plan/resource 报告及固定 DINOv2 的一次实际 Neural Engine 分配已经关闭；峰值内存/热量、模型安装、
自动后台重训、生产标准包接线和真实照片只读 smoke
仍为后续独立验收门。
...
revision 门；自动后台重训与生产模型安装继续分别验收，未来 production standard artifact 仍须重做
