...
状态：Slice A implemented。连接、授权、单 Photos 来源、持久化分批元数据对账、统一网格/来源筛选和
本地限定缩略图已在 `4f1e1a5` 完成并通过 732 项完整测试。persistent change history、来源恢复和显式
iCloud 获取仍属于 Slice B/C。实施顺序与验收边界见
...
| ADR-016 | 阶段 2 使用单 Photos 来源、统一网格和显式本地限定内容请求 | 已决定 | 复用现有三栏工作台；默认不触发 iCloud 下载，保持 Photos 源端零写入 |
