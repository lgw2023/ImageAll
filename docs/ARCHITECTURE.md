...
preprocessing/postprocessing 语义身份的 App 内 embedding cache 均已关闭；当前下一门是缓存资源与
生命周期窄门，之后进入 App 内轻量个人线性 head。
...
| ADR-031 | App 内 DINO embedding 采用完整语义身份的自有 Caches 文件缓存 | 首个显式单图 tracer 已实现 | key 绑定 catalog/asset/content、encoder/preprocessing/postprocessing、元素语义及 artifact provenance；值只含有限 little-endian Float32、身份与 SHA-256，损坏/旧身份 miss，持久化失败退化为实时 embedding，不保存图片、路径或 bookmark；下一门补跨实例发布与有界回收 |
