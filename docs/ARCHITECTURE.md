...
SHA-256；在官方字节复核、许可义务和公开数据 manifest 完整前保持 `research`，不得替换现有 fixture。
...
| ADR-025 | 标准标签使用版本化 ontology DAG 和独立公共模型生命周期 | ontology/model identity 与祖先展开已实现；生产准入规格已冻结，Places365 候选仍为 research | 一个概念可属于多个父类；model、mapping、policy 与 ontology 分别版本化，冲突安装不迁移人工决定；来源、许可、公开评测、转换和资源证据全部通过前不得生成生产包 |
