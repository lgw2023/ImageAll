...
→ 从来源行重扫、重新授权或按 ADR-049 从 ImageAll 删除来源
→ 从标签行重命名或归档标签，同时保留人工决定历史
→ 活跃文件夹发生变化后自动合并事件、重跑对账并刷新网格
```

后续模型能力继续遵循同一纵切片原则，但正式部署已固定为 App 容器内 Swift 直接加载 Core ML；
Python/uv loopback 只作转换、离线评测和开发验证，不属于用户主路径。Developer ID 与 Mac App Store
的选择不阻塞该接线，模型未启用或 artifact 失效不得影响本阶段已交付的浏览与人工标签闭环。
...
- 来源行使用原生上下文菜单提供“在图库中查看”“立即重扫”“重新授权…”和“删除来源…”；重新授权只在 `unavailable` / `authorizationRequired` 状态可用，重扫只在 `active` 状态可用；
...
Stage 4-W 已由 `bea02c5` 恢复百万资产搜索余量：v006 使用 FTS5 trigram 缩小文件名/相对路径候选，
再以原 LIKE 复核字面子串语义；来源和标签仍从各自事实表预解析。干净提交快照全量 811/811 通过，
1 万与 100 万六项查询分别为 0.012 秒和 0.209 秒，其中搜索为 0.001 秒和 0.129 秒；签名 entitlement
测试与严格验签 Debug build 通过。结果、排序、分页、阈值和真实照片 I/O 边界均未改变。

下一推荐切片转入 Stage 4-X：量化百万级既有 v005 目录库升级到 v006 的耗时和峰值工作副本空间。
Slice W 只用小 fixture 证明回填正确，100 万查询完成后的数据库与 sidecar 已从 2,168,393,928 字节增加到
2,360,339,760 字节，尚不能证明当前启动容量预检覆盖 migration 期间的快照、工作副本、WAL 与 FTS 回填峰值。
该切片应只使用 opt-in 合成 v005 数据库，验证 migration history、Asset/FTS 数量与匹配、`quick_check`、
失败时原库保持不变，并记录迁移耗时及前后/峰值空间；证据不足前不冻结跨设备 SLA，也不把百万 fixture
加入每次默认全量测试。不读取图片字节，不调用 PhotoKit，不读取 `user/`，不访问或遍历 `/Volumes/HDD2`。
