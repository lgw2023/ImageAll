...
struct AsyncFeatureVectorBridge: SyncFeatureVectorLoading, @unchecked Sendable {
    private final class ResultBox: @unchecked Sendable {
        private let lock = NSLock()
        private var value: Result<FeatureVectorPayload, Error>?

        func store(_ value: Result<FeatureVectorPayload, Error>) {
            lock.lock()
            self.value = value
            lock.unlock()
        }

        func load() -> Result<FeatureVectorPayload, Error>? {
            lock.lock()
            defer { lock.unlock() }
            return value
        }
    ...
    let loader: any FeatureVectorLoading
    ...
    func loadOrGenerateSync(assetID: UUID) throws -> FeatureVectorPayload {
        ...
        Task {
            do {
                box.store(.success(try await loader.loadOrGenerate(assetID: assetID)))
            } catch {
