...
struct GRDBFolderSourceAuthorizationRepository: Sendable {
    ...
    func fetchFolderSource(id: UUID) throws -> StoredFolderSourceRecord? {
        try database.pool.read { db in
            try fetchFolderSource(db, id: id)
        }
    }
    ...
    func disableFolderSource(sourceID: UUID, nowMs: Int64) throws {
        try database.pool.write { db in
            guard try fetchFolderSource(db, id: sourceID) != nil else {
    ...
    private func fetchFolderSource(_ db: Database, id: UUID) throws -> StoredFolderSourceRecord? {
        ...
        return try storedFolderSource(from: row)
    }
