...
def _benchmark_evidence(payload: dict[str, Any]) -> dict[str, object]:
    ...
    return {
        ...
        "encoder": {**expected_encoder, "model_sha256": model_sha256},
    }
...
def _ane_evidence(root: ET.Element) -> dict[str, object]:
    ...
    for row in node.findall("./row"):
        label_element = row.find("./formatted-label")
        label = "" if label_element is None else "".join(label_element.itertext())
        if (
            _optional_element_text(row.find("./ane-event-name"))
            == "Apple Neural Engine"
            and _optional_element_text(row.find("./gpu-state")) == "Active"
            and "AneInference" in label
        ):
            duration_text = _optional_element_text(row.find("./duration"))
            try:
